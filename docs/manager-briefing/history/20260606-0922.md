...
| F | supplier scanner | Blocked | 78% | F Seller Central/Dashboard Yes-No proof has cleared. The script reached the Seller Central code step, used the code path, and BBP Dashboard Yes/No is now being read as `YES`. The remaining F fail is the older protected rescan-priority row, not this login task. | Keep F-RESCAN-PRIORITY-02 visible as a protected Luke decision. |
...
| M | main manager | Warning | 67% | Current Manager Task Board: 25 active cards, 2 not started, 1 in progress, 3 blocked, 19 parked. | Keep warning visible and continue only if it blocks today's restocking work. |
