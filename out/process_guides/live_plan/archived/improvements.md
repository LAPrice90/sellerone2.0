## High-level read of what you’re building

You’re designing a repricing system that explicitly separates:

* **Market truth** (all offer instances, including duplicate offers from the same seller) and **decision truth** (who actually matters, and what price wins in practice). 
* **Market-with-us vs market-without-us** so you don’t fool yourself when you already hold the Buy Box. 
* **Price-only vs “effective price”** where delivery speed is converted into a GBP penalty and included in the competitive comparison. 
* **SKU strategy vs seller strategy** (SKU profile + SKU–seller profiles) so one aggressor doesn’t contaminate logic for every competitor.
* **Controlled execution** via the Head/Supervisor/Executioner stack and a dedicated H pricing cycle, with PPP staying as the broad executor while your lab learns behaviour on a tiny cohort.

That is the right architecture for avoiding “repricer brain damage” (thrash + blind spots) and for turning your manual “war story” into measurable rules.

Below are the main holes that will bite you, and the upgrades that close them.
...
Right now the default funnel aggregation is “take the minimum required winning price across selected sellers”  — that implicitly pushes you toward the *lowest* feasible price whenever there is even one hard-to-beat seller in the selected set.
...
You already have “expected_profit_per_day_if_competitive” as a concept field  — the hole is that it isn’t the decision engine’s *actual* optimizer yet.
...
* Delivery promises are **customer-location dependent**. FOEP itself explicitly warns featured offer is not guaranteed and can vary with fulfillment capabilities and customer location. ([GitHub][1])
...
* Treat delivery penalty as **two layers**:
  ...
  1. **Bootstrap heuristic** (simple, stable, safe):

     * e.g., penalty by gap days with a cap (you already outline a starter curve).
  2. **Learned correction** that updates ostrong.
* Use **SP-API pricing primitives to help bootstrap**:

  * FOEP is literally “computed listing price at or below which a seller can expect to become the featured offer”, but it’s not guaranteed and is sensitive to other factors (including fulfillment capabilities). ([GitHub][1])
  * Amazon’s own SP-API pricing solution materials describe workflows for retrieving featured offer/price thresholds and even regionalized featured offer info by location. ([developer.amazonservices.com][2])
...
1. **Buy Box eligibility ceiling** (how high you can be and still plausibly win featured offer), using FOEP and CompetitivePriceThreshold.
2. **Market realism / demand ceiling** (how high customers will still buy, even if you’re alone).

And in practice there’s a third:

3. **Compliance ceiling** (how high you can price without risking suppression / account-health consequences tied to Amazon’s pricing policies).

Why this mattersThreshold (CPT) is explicitly tied to Amazon comparing offers to reputable retailers outside Amazon, and they only feature offers when price is at or below that external benchmark. ([developer.amazonservices.com][3])
...
Amazon’s own pricing guidance explicitly states they may remove the Featured Offer or offer (and more severe actions) if pricing practices could damage customer trust. ([Sell on Amazon][4])
...
* A systematon machine** that will sometimes fight the wrong opponent (e.g., Amazon Retail, a non-reactive seller, a seller with structurally lower costs).
* Incg caught in **pricing health / featured offer ineligibility loops** (CPT/external reference pricing can disqualify you even when you’re lowest on Amazon). ([developer.amazonservices.com][3])
...
* Use two IDs:
  ...
  * `offer_snapshot_id` = unique per observation row (time-based).d` = stable per seller/fulfilment/condition/shipping template *without* including the current price.
...
This is correct because SP-API  and dynamic, and Amazon explicitly recommends reducing redundant polling and using Notifications for event-driven architecture. ([GitHub][5])

Two practical improvements here:

* Treat `PRICING_HEALTH` and `ANY_OFFER_CHANGED` as **complementary** not substitutable: Amazon’s own SP-API materey trigger in different situations and you should adopt both. ([GitHub][6])
* Add rate-limit observability early:

  * monitor rate limits via headers, and enforce per-SKU API budgets in the scheduler. ([GitHub][5])
...
* FOEP was extended to Amazon.co.uk, but still has eligibility constraints (new condition, ship nationwide, eligible to become featured offer) and returns statuses like `ASIN_NOT_ELIGIBLE`. ([GitHub][7])
...
Amazon’s SP-API materials describe `getCompetitiveSummary` returning (in one call) featured buying options, lowest priced offers, and reference prices, and explicitly mention CompetitivePriceThreshold usage. ([GitHub][6])

That aligns directly with your “market truth” requirement and will simplify your A-side/H-side architMake “single writer per SKU” a non-negotiable invariant
...
1. **Add a “Compliance & Account-Health Guardrail” section** near Guardrails:

   * Explicitly reference that Amazon can remove Featured Offer / offer for pricing practices that damage customer trust. ([Sell on Amazon][4])
   * Add “pricing-health disqualification” as a trigger override and as a learning-data disqualifier.

2. **Split “ceiling” into 3 fields** (compliance / eligibility / demand) and update the clamp rule accordingly.

3. **Change the funnel aggregation default** from “minimum required price” to “maximize expected profit under constraints,” while keeping “win-all” as an explicit, rarely-used policy.

4. **Add outcome-quality gates** for delta learning updates (invariants) so the learner doesn’t poison itself.

5. **Define two IDs for offers** (snapshot vs variant) to make your “instance-safe” rule analyzable.

---

## Priority order: what to fix first (highest leverage)

1. **Define the decision objective** (profit/day with constraints) and stop defaulting to “min price wins.”
2. **Outcome truth + invariant tagging** (so your delta learnege).
3. **Ceiling split** (eligibility vs demand vs compliance) + clamp order.
4. **Notifications + rate-limit observability** behe pilot (your own gate is correct).
5. **Keep “pressure/nuclear” mancan prove net profit uplift after fight costs.

If you wantmaster file sections as a patch (same structure, but with these additions inserted in-place), but the points above are the actual holes that matter.



[1]: https://github.com/amzn/selling-partner-api-models/discussions/3489 "January 2023 SP-API Release Announcement · amzn selling-partner-api-models · Discussion #3489 · GitHub"
[2]: https://developer.amazonservices.com/solutions-automated-pricing-management-on-amazon "Amazon Selling Partner API (SP-API)"
[3]: https://developer.amazonservices.com/october2021 "Amazon Selling Partner API (SP-API)"
[4]: https://sell.amazon.com/blog/amazon-pricing-strategies "Amazon pricing strategies: How to price products to drive sales - Sell on Amazon"
[5]: https://github.com/amzn/selling-partner-api-samples/discussions/102 "Preparing your SP-API Application for Peak Shopping Events ️ · amzn selling-partner-api-samples · Discussion #102 · GitHub"
[6]: https://github.com/amzn/selling-partner-api-samples/discussions/100 "Using SP-API to improve Price Competitiveness  · amzn selling-partner-api-samples · Discussion #100 · GitHub"
[7]: https://github.com/amzn/selling-partner-api-models/discussions/3733 "November 2023 SP-API Release Announcement · amzn selling-partner-api-models · Discussion #3733 · GitHub"
