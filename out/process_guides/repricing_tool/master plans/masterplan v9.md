# Masterplan v9 - Unified Repricing Intelligence + Delivery Value Engine + Three-Ceiling Governance + Pressure State + Dual-Identity Offer Model + Outcome Admissibility + Event-Led Runtime + Eligibility Ceiling Ladder + Runtime Safety Addendum

Status: Active Draft (Unified v9)  
Date: 2026-02-13  
Owner: Luke (business intent), Codex (execution support)

## 0) What Changed vs v8 (diff guide)
1. Added runtime safety addendum sections (single-writer arbitration, share-units stabilisation, variant identity fallback, OAS hash tightening).
2. Added a pre-expansion hardening checklist that must pass before multi-SKU rollout.
3. Preserved all v8 canonical architecture and constraints; v9 is v8 plus these mandatory hardening controls.

## 0A) What Changed vs v7 (historical carried forward)
1. **Notifications are now a core dependency (not a â€œpre-scale enhancementâ€)**
   - The runtime is event-led by default: notification intake â†’ dedupe â†’ refresh orchestration â†’ targeted snapshot â†’ decision.
...
- Learns from **real Buy Box outcomes** and **seller behaviour**, not just visible â€œlowest priceâ€.
...
## 3) Non-Negotiables
1. **Default objective is profit optimisation**: never â€œlowest winning priceâ€ by default.
...
5. **Ceilings clamp before optimisation**: the optimiser never sees â€œforbiddenâ€ price space.
...
- keys:
  ...
  - `timestamp_utc`
  - `seller_id_raw`
  - `seller_id_canonical` (derived; see Â§5.1.3)
  - `offer_variant_id` (derived; see Â§5.1.2)
- offer facts:
...
- snapshot context:
  ...
  - market structure fields (see Â§14)

Rules:
- **Never dedupe by `seller_id` only.**
- **Never include price or delivery days in any â€œidentity keyâ€.**
...
- plan:
  ...
  - `max_daily_cut_gbp`
  - `cooldown_days_after` (default 3â€“7)
  - `exit_triggers[]`
  - `kill_conditions[]`
- economics:
...
#### A-Cycle (Daily Intelligence Build) â€” once/day per SKU
...
#### H-Cycle (Execution / Optimisation) â€” event-led runtime (notifications first) + heartbeat polling fallback
Triggered by:
- **Notifications (preferred):**
  - `ANY_OFFER_CHANGED` â†’ tactical refresh targeting (only refresh SKUs that actually moved)
  - `PRICING_HEALTH` â†’ risk interrupt (override state + clamp/freeze logic)
- **Heartbeat polling (fallback only):**
  - low-frequency â€œare we blind?â€ checks to cover missed notifications or integration outages
  - automatic escalation if `notifications_last_seen_utc` is stale

Execution order (runtime envelope; pre-decision orchestration):
0. **Consume event inbox** â†’ dedupe â†’ apply per-SKU cooldown â†’ create refresh request with urgency (`now|soon|defer`)
1. **Budget gate** (global + per-SKU) â†’ prioritise `PRICING_HEALTH` over routine refresh
...
- Â§5.2 defines trigger intake, queueing, budget gating, and refresh orchestration.
- Once a refresh is admitted for execution, the decision pipeline must follow **Â§11 Step 0 -> Step 14** in exact order.
- If wording ever conflicts between Â§5.2 and Â§11, **Â§11 is canonical for per-refresh decision order**.
...
### 7.2 Layer 1 â€” Bootstrap Heuristic (Always On)
Default conservative curve (v0):
- gap 0 days â†’ Â£0.00
- gap 1 day  â†’ Â£0.15
- gap 2 days â†’ Â£0.30
- gap 3 days â†’ Â£0.45
- gap 4+ days â†’ Â£0.60 (cap)
...
### 7.3 Layer 2 â€” Eligibility Intelligence Overlay (FOEP + market + deterministic fallbacks)
Eligibility intelligence does not write prices. It acts as:
1) **constraint**: clamps candidate price space so the optimiser never â€œbelieves inâ€ infeasible Featured/Buy Box pricing
2) **diagnostic**: tells you when you are losing eligibility for structural reasons (not just competition)

Daily persisted fields (see Â§5.1.6):
...
Core rule:
- FOEP is **optional**. Missing/invalid FOEP must not widen the optimiserâ€™s search space; it must trigger a reason-coded fallback path.
- CPT is **not** a hard ceiling in Phase 1. It is logged, monitored, and used for risk classification only.

See Â§8.2 for the full eligibility ceiling ladder and FOEP usability rules.
...
- `market_structure_hash` stable during those events (variant-level stability; see Â§14)
...
### 8.1 Compliance Ceiling (Policy / Suppression Safety) â€” landed-price
Inputs:
- `external_reference_price_gbp` (optional)
- `policy_buffer_pct` (recommended 0.02â€“0.05)
...
### 8.2 Eligibility Ceiling (Buy Box / Featured Offer feasibility) â€” landed-price
...
Key principle:
- FOEP is **optional intelligence** with explicit failure modes. Missing/invalid FOEP must **not** widen the optimiserâ€™s allowed space; it must trigger a deterministic fallback path with reason codes.
...
If one side is missing, the other may bind â€” but the missing side must be reason-coded and confidence-adjusted.
...
- sanity check passes (starter):
  - FOEP is within plausible bounds vs observed market (e.g. within `[0.5Ã—, 2.0Ã—]` of current best-rival landed price, if rivals exist)
...
Fallbacks:
- If rivals absent â†’ market-based ceiling may be null (signal ladder must cover the SKU).
- If delta confidence too low â†’ use conservative heuristic delta and reason-code:
...
### 8.3 Demand Ceiling (Conversion / Volume Realism) â€” effective-price first
...
1) If a pressure case is `active` â†’ `state = pressure`
2) Else if competition absent â†’ `state = low_no_competition`
3) Else if volatility / integrity risk high â†’ `state = defensive_hold`
4) Else â†’ `state = maximize_profit`
...
## 10) Pressure State (Nuclear Mode) â€” Full Integration
...
- CPT/eligibility anchoring risk (you permanently drag the listingâ€™s future ceiling down)
...
#### Gate 1 â€” Opponent validation (do not fight structurally stronger sellers)
...
- `reaction_speed_minutes <= reaction_speed_max` (must be â€œfast chaserâ€)
...
If opponent is structurally stronger â†’ **disqualify**.
...
#### Gate 2 â€” Profit uplift model (expected gain must be > 0)
Compute:
...
expected_gain =
(probability_of_exit Ã— expected_uplift_days Ã— uplift_profit_per_day)
-
(expected_pressure_duration_days Ã— margin_loss_per_day)
```

Rule:
- if `expected_gain <= 0` â†’ **abort**
- no emotion allowed

#### Gate 3 â€” Resource check (you must be able to sustain the campaign)
All must be true:
...
#### Gate 4 â€” Compliance + CPT anchor distortion check (protect future ceiling)
...
- if the campaign would create a new â€œtoo-low anchorâ€ relative to recent history â†’ abort unless Head overrides with explicit reason code.
...
- surface â€œdo not fightâ€ warnings with reason codes
...
### 10.8 Evidence requirement before any â€œpartial automationâ€
Pressure execution remains manual until:
- **20â€“30 completed pressure cases** exist
...
- Event intake, dedupe, cooldown, queueing, and budget admission are handled earlier by Â§5.2 and Â§18.2-Â§18.3.
- After admission, run Step 0 -> Step 14 exactly as written below.

### Step 0 â€” Strategy State Resolution
- If `pressure_case_status = active` â†’ follow pressure plan
- Else choose state using Â§9 rules

### Step 1 â€” Portfolio Governor Gate (Mandatory Daily)
...
### Step 3 â€” Market Truth Snapshot (Offer Snapshot Facts)
...
### Step 4 â€” Variant Mapping (Mandatory)
...
- link snapshot row â†’ variant id
...
### Step 11 â€” Outcome + Profit Estimation (Normal states only)
...
### Step 12 â€” Guardrails + Risk Caps
...
### Step 14 â€” Observe + Update Learning (If Stable)
...
If OAS blocks the update, **still log the outcome** (with contamination reasons) and leave memory unchanged. See Â§14.
...
- **Market Structure** (who is present + their structural identity) â€” should be mostly stable during a probe window.
- **Market Posture** (prices, Buy Box rotation, delivery windows) â€” can change, but changes may contaminate a probe.
...
### 14.2 Outcome Admissibility Service (OAS) â€” Mandatory for Any Learning Update
...
#### Hard invariants (any violation â‡’ `context_quality_score = 0`)
...
### 14.4 `context_quality_score` â€” Scoring (starter, explicit, tunable)

Start at 1.0 and apply multipliers (never exceed [0,1]).

**Hard fails â†’ 0.0 immediately.**

Suggested starter multipliers:
- Minor competitor drift â†’ Ã— 0.7
- Moderate drift â†’ Ã— 0.4
- Delivery posture shift (minor) â†’ Ã— 0.7
- Buy Box rotation noise detected â†’ Ã— 0.6

**Quality bands:**
- `0.85â€“1.00` = clean (update bounds + estimate)
- `0.40â€“0.85` = usable but noisy (update estimate; bounds update only if strong evidence)
- `0.01â€“0.39` = weak (log only; optionally update estimate with very small weight)
- `0.00` = inadmissible (no updates)

### 14.5 Update Rule (Quality-Weighted) â€” What Actually Changes in Memory
...
Recommended fields:
- `highest_delta_win_effective_gbp` (hard bound; updates only if `context_quality_score â‰¥ 0.85` and win observed)
- `lowest_delta_loss_effective_gbp` (hard bound; updates only if `context_quality_score â‰¥ 0.85` and loss observed)
...
Mandatory delta memory fields (see Â§5.1.4):
...
- `DELTA_UNKNOWN` (no bracket yet) â†’ seek first win/loss with small safe probes
- `DELTA_BRACKETING` (one side known) â†’ explore until both bounds exist
- `DELTA_BRACKETED` (both bounds exist) â†’ narrow until width â‰¤ `delta_tolerance_gbp`
- `DELTA_CONFIDENT` (high confidence) â†’ re-test daily or on drift
- `DELTA_DEGRADED` (contamination ratio high / stale) â†’ reduce trust, re-learn
...
### Stage A â€” Pilot SKU live learning (controlled)
Implement:
...
### Stage B â€” Three-ceiling enforcement (hard)
...
### Stage C â€” Profit-led decision layer (default)
Profit engine becomes default selection rule.

### Stage D â€” Pressure recommendation only (no execution)
...
### Stage E â€” Manual pressure case workflow (time-boxed)
- execute only with Head approval
- log complete pressure cases and outcomes
- target: 20â€“30 cases before any discussion of partial automation

### Stage F â€” Eligibility intelligence (A-cycle)
- FOEP + CPT pulls as daily baseline.

### Stage G â€” Demand ceiling learning (slow)
- enable demand ceiling model in effective-price space with strict stability gating.

### Stage H â€” DVE learned correction (multiplier)
- enable multiplier learning with strict evidence rules.

### Stage I â€” Event-led runtime hardening (pre-expansion, mandatory)
...
- low-frequency heartbeat polling fallback (â€œare we blind?â€ checks)
...
## 17) Appendix â€” Reason Codes (Starter Set)
...
### 18.1 Outcome Admissibility Layer (core; see Â§14)
Outcome admissibility is no longer an add-on. It is specified in **Â§14 (Learning Integrity + Outcome Admissibility)** and is mandatory for any learning update.
...
- maintain a per-SKU â€œevent inboxâ€
- dedupe by `(sku, event_type, short_time_window)` to prevent stampedes

#### 18.2.2 Event â†’ refresh orchestration (Supervisor responsibility)
For each SKU with events, decide urgency:
- `now` (immediate)
- `soon` (batch within 15â€“30m)
- `defer` (batch later)

Apply storm control:
- per-SKU refresh cooldown (â€œat most 1 refresh per X minutes unless PRICING_HEALTHâ€)
...
Low-frequency â€œare we blind?â€ checks exist to cover:
- missed notifications
- integration outages
- stale inbox health

Heartbeat cadence is lane/state-derived:
- `parked` / ignore lane â†’ 4â€“8h
- `monitor` â†’ 1â€“4h
- `duel` / active fight â†’ 15â€“60m (still event-led; polling is backup)
...
- `ANY_OFFER_CHANGED`
  - triggers targeted refresh with urgency based on cadence profile:
    - `duel` / `wait_out` â†’ `now`
    - `monitor` â†’ `soon`
    - `parked` â†’ ignore unless seller-of-interest re-entry or explicit rule match
...
Scaling fails when â€œrefresh everythingâ€ hits rate limits and the system silently becomes stale.
...
- `calls_used_today_by_sku` distribution (detect â€œhogâ€ SKUs)
...
If FOEP disappears and the system treats that as â€œno ceilingâ€, the optimiser will:
...
Source ladder (strict order) is defined in Â§8.2.2:
`FOEP â†’ MANUAL â†’ HIST_WIN â†’ RIVAL_PLUS_DELTA / LAST_KNOWN_SAFE`
...
These outputs are required and are listed in Â§15. This subsection exists as a reminder that runtime hardening is not optional.
...
Below is a **v9 Patch** designed to be appended to *Masterplan v9* without rewriting any existing sections.
...
# v9 Patch â€” Runtime Safety + Optimiser Stabilisation Addendum

Status: Canonical Addendum in v9
...
```
pricing_writer_mode âˆˆ {PPP, CODEX_H, READ_ONLY}
...
rank_based_estimate
Ã— buy_box_win_rate_30d_adjustment
```
...
if effective_gap <= -delta_confident:
    share â‰ˆ high_share_cap (e.g., 0.7â€“0.9)
elif effective_gap between (-delta, +delta):
    share â‰ˆ mid_share_band
else:
...
```
shipping_template_confidence âˆˆ {HIGH, MEDIUM, LOW, UNKNOWN}
```

Add per variant:

```
variant_integrity_score âˆˆ [0,1]
```
...
Stabilises learning â†’ stabilises pricing.
...
# Final Assessment After v9 Patch
...
At that point, the plan is no longer just â€œintelligentâ€.
...
* Produce a **minimal implementation order (build sequence)** so you donâ€™t try to build everything at once,
