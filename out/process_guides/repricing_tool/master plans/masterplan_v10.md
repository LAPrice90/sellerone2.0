# Masterplan v10 - Unified Repricing Intelligence + Delivery Value Engine + Three-Ceiling Governance + Pressure State + Dual-Identity Offer Model + Outcome Admissibility + Event-Led Runtime + Eligibility Ceiling Ladder + Buy Box Suppression Reactivation + Runtime Safety Addendum

Status: Active Draft (Unified v10)  
Date: 2026-03-07  
Owner: Luke (business intent), Codex (execution support)

## 0) What Changed vs v9 (diff guide)
1. Added Buy Box suppression as a first-class eligibility state, not as "missing Buy Box data".
2. Added canonical `buy_box_state` classification:
   - `NORMAL`
   - `LOST_TO_COMPETITOR`
   - `SUPPRESSED_ASIN`
   - `DISQUALIFIED_SELF_PRICE`
   - `UNKNOWN`
3. Added `STATE_SUPPRESSION_REACTIVATION` as a dedicated strategy state separate from competition logic and pressure logic.
4. Added `suppression_ceiling_landed_temp` as a temporary eligibility ceiling clamp that never widens price space and must expire.
5. Added a strict suppression reactivation target ladder that always prefers direct signals before probing.
6. Added bounded suppression threshold discovery with `highest_eligible_price`, `lowest_ineligible_price`, `suppression_threshold_estimate`, `suppression_threshold_confidence`, and `suppression_last_validated_utc`.
7. Added `anchor_floor_price` to protect long-term price anchors during suppression recovery.
8. Added suppression learning isolation so suppression windows cannot contaminate competitor delta learning, aggressor classification, or demand elasticity models.
9. Integrated suppression into eligibility intelligence, the three-ceiling clamp, the strategy state machine, the H-cycle decision flow, OAS, logging, reason codes, health checks, and rollout staging.
10. Preserved all major v9 sections and runtime hardening controls; v10 is v9 fully carried forward plus suppression architecture.

## 0A) What Changed vs v7 (historical carried forward)
...
4. **Eligibility ceiling is now a deterministic ladder with explicit FOEP failure modes**
   ...
   - Phase 1 update: CPT is pinned and observed as a risk signal, not used as an eligibility ceiling clamp.
   - When FOEP is missing/ineligible/untrusted, the system falls back in a reason-coded order (manual -> historical wins -> conservative market proxy).
...
## 0B) Phase 1 Runtime Deviations (temporary)

These are known temporary Phase 1 limits and are not treated as design conflicts.

1. CPT handling (Phase 1)
   - CPT may still be used in Phase 1 code as a safety compliance ceiling only.
   - CPT must not be used as a demand ceiling or max-price target.
   - In later phases, CPT is telemetry/risk and not a ceiling unless explicitly enabled as an empirical model.

2. Strategy segmentation (SNL vs STANDARD)
   - Segmentation is mandatory at portfolio scale.
   - Phase 1 may run with a single default strategy tag until multi-SKU onboarding is implemented.
   - Segmentation is required before expansion beyond the pilot cohort.

3. Demand ceiling learning / profit peak
   - Out of scope for Phase 1.
   - Phase 1 uses `manual_cap_price_gbp` as a demand proxy.
   - Profit/day peak tracking is introduced in later phases after execution stability.
...
- Treats Buy Box suppression as an **eligibility state** that must be classified and managed, not as missing data.
- Restores Buy Box eligibility safely when Amazon suppresses the Featured Offer due to price or related eligibility failure.
- Prices the economic reality that:
  - **Delivery speed has monetary value** (Delivery Value Engine).
  - **Eligibility signals constrain feasible pricing** (FOEP + market structure; CPT is risk telemetry only in Phase 1).
...
12. **Eligibility intelligence never writes prices**: FOEP and market signals constrain; CPT warns/classifies risk and does not set targets in Phase 1.
...
17. **Strategy segmentation is mandatory per SKU**:
    - every SKU must be tagged `SNL` (small-and-light) or `STANDARD`
    - strategy tag controls cap movement, expansion speed, and fee-cliff behavior
    - strategy tag is set at onboarding and stored as first-class config

### 3A) Mandatory Dual Strategy Framework (Portfolio Scale Safety)

#### 3A.1 SKU classification rule
```
if small_and_light:
    strategy = SNL
else:
...
#### 3A.2 SNL strategy (small-and-light, sub-GBP10 zone)
- Objective: stable volume plus fee optimization, not max theoretical price.
- Fee cliff rule:
  - treat GBP9.99 as a structural ceiling by default.
  - crossing GBP10+ is allowed only when post-fee margin is proven higher and velocity is not degraded.
- Cap movement:
  - upward probe steps must be small (GBP0.10 to GBP0.20).
  - observation windows must be longer before expanding cap again.
- Elasticity guard:
  - meaningful velocity drop must reduce cap immediately.
  - no aggressive cap expansion when elasticity is unstable.
- CPT handling:
  - CPT is context and risk telemetry only.
  - low CPT is a confidence brake on cap expansion, never a hard clamp.

#### 3A.3 STANDARD strategy (higher-ticket / non-SNL)
- Objective: maximize sustainable profit/day with controlled expansion.
- Cap movement:
  - upward steps can be wider (GBP0.50 to GBP1.00) when market conditions are clean.
  - faster expansion is allowed when competition is weak/absent.
- Demand ceiling:
  - learned dynamically from realized profit/day response.
  - continue expansion until profit/day degrades beyond tolerance.
- CPT handling:
  - CPT is an extreme sanity/risk signal only.
  - ignored for day-to-day cap setting unless severe drift risk is detected.
- Inventory coupling:
  - deep stock should avoid over-tight caps that choke velocity.
  - thin stock can tolerate a stretched cap for margin capture.

#### 3A.4 Unified parameter matrix
- `SNL`: small cap step, fee-cliff aware, low elasticity tolerance, slow cap expansion, CPT light brake.
- `STANDARD`: medium cap step, fee-cliff mostly irrelevant, medium elasticity tolerance, moderate cap expansion, CPT very light brake.
...
- `competitive_price_gbp` (external or Amazon-provided competitive reference where available)
- `average_selling_price_gbp` (where available)
- `cpt_last_refresh_utc`
- `buy_box_state`
- `buy_box_state_confidence`
- `buy_box_eligible_offers`
- `pricing_health_active_flag`
- `pricing_health_reason_codes[]`

**Derived ladder output (source-of-truth eligibility ceiling)**
- `eligibility_ceiling_landed_gbp`
- `eligibility_source` (`FOEP|MANUAL|HIST_WIN|RIVAL_PLUS_DELTA|LAST_KNOWN_SAFE`)
- `eligibility_confidence`
- `eligibility_reason_codes[]`
- `eligibility_last_refresh_utc`

**Suppression-specific derived intelligence**
- `suppression_detection_state` (`NONE|SUPPRESSED_ASIN|DISQUALIFIED_SELF_PRICE|UNKNOWN`)
- `suppression_reactivation_target_landed_gbp`
- `suppression_target_source` (`CPT|COMPETITIVE_PRICE|AVERAGE_SELLING_PRICE|FOEP|PROBE_BRACKET`)
...
- Buy Box suppression is handled as an eligibility condition inside this layer, not as missing data.
- Direct signals must always outrank suppression probing.
...
- `suppression_ceiling_landed_temp`
- `binding_ceiling_type` (`compliance|eligibility|demand|manual|suppression_temp`)
...
- Pull or carry forward suppression reference signals such as `competitive_price` and `average_selling_price` where available.
...
- Refresh suppression threshold memory only from admissible suppression cases.
...
- **Notifications (preferred):**
  ...
  - `PRICING_HEALTH` -> risk interrupt and suppression-classification trigger where relevant
...
5. **Classify Buy Box state** (`NORMAL|LOST_TO_COMPETITOR|SUPPRESSED_ASIN|DISQUALIFIED_SELF_PRICE|UNKNOWN`)
6. **Compute snapshot-specific ceilings** (Ceiling Engine; incl. eligibility ladder output + market-based ceiling + suppression clamp when active)
7. **Resolve Strategy State** (normal vs suppression reactivation vs pressure) and apply any `PRICING_HEALTH` overrides
8. **Decide action**
   - normal: run profit optimisation inside floors/ceilings
   - suppression: run direct-signal reactivation or bounded threshold discovery inside guardrails
   - pressure: follow the approved pressure plan schedule (no optimisation)
9. **Propose/execute action** (approvals + guardrails)
10. **Log + observe + update learning (if stable)** via OAS
...
- **All ceilings ultimately clamp our landed price** (because Amazon pricing + listing price are landed-price constructs), but:
  ...
  - suppression creates a temporary landed-price eligibility clamp inside the same ceiling system

### Buy Box state rule (canonical)
`buy_box_state` must be one of:
- `NORMAL`
- `LOST_TO_COMPETITOR`
- `SUPPRESSED_ASIN`
- `DISQUALIFIED_SELF_PRICE`
...
Rules:
- suppression is an eligibility state, not missing data
- `UNKNOWN` is an observability state and must not trigger suppression probing
- `LOST_TO_COMPETITOR` remains competition logic, not suppression logic
...
### 7.3 Layer 2 — Eligibility Intelligence Overlay (FOEP + market + deterministic fallbacks)
DVE remains a competitiveness translation layer. It must not reinterpret direct suppression evidence as a delivery-only issue. When `buy_box_state` indicates suppression, DVE informs effective-price comparisons but eligibility restoration logic still binds first.
...
- CPT risk telemetry: `cpt_gap_vs_buybox_gbp`, `cpt_gap_vs_avg_sold_gbp`, `cpt_risk_class`, `cpt_drift_7d_gbp`
...
- CPT is **not** a hard ceiling in Phase 1. It is logged, monitored, and used for risk classification only.
...
Computation:
```text
compliance_anchor = external_reference_price_gbp?
...
- If compliance inputs are missing, mark low confidence and fall back in this order:
  - use last known compliance ceiling
  - else escalate (`FAIL_NO_SAFE_COMPLIANCE_CEILING`)

Phase 1 CPT handling (pin + observe):
- CPT can be below live Buy Box and is not treated as a reliable profit-max ceiling.
- Being above CPT can increase Featured/Buy Box eligibility risk, but there is no deterministic "CPT + X" safe buffer.
- If `hard_floor_price_gbp > competitive_price_threshold_gbp`, classify the SKU as `CPT_INCOMPATIBLE` and log risk; do not force price down to CPT.
...
- Buy Box suppression is an **eligibility state**. It must tighten feasible space, not be treated as null data.

Eligibility ceiling is computed from three connected families:

1) **Signal-based eligibility ceiling (SKU-level, A-cycle, persisted)**
- Derived from FOEP and deterministic fallbacks.
...
3) **Suppression temporary ceiling (event-level, H-cycle and carried forward briefly)**
- Derived from direct suppression reference prices or a validated probe bracket.
- Stored as `suppression_ceiling_landed_temp`.
- This ceiling never widens space and always expires.

**Runtime rule (H-cycle):**
```text
eligibility_ceiling_landed_h =
  min(
    eligibility_ceiling_landed_signal? ,
    eligibility_ceiling_landed_market? ,
    suppression_ceiling_landed_temp?
  )
```
...
- sanity check passes (starter):
  - FOEP is within plausible bounds vs observed market (e.g. within `[0.5x, 2.0x]` of current best-rival landed price, if rivals exist)
...
1. **FOEP (usable)**
2. **Manual ceiling** (Head-provided manual cap, reason-coded)
3. **Historical highest winning effective band** (from admissible wins; converted to landed per snapshot as needed)
4. **Conservative market proxy** (e.g. `best_rival_effective - heuristic_delta`, or last-known-safe ceiling)
...
Important:
- CPT still must be persisted daily and reason-coded as risk telemetry (`CPT_BELOW_CLEARING_PRICE`, `CPT_DRIFT_DOWN`, `CPT_INCOMPATIBLE`) when observed.
- Optional future mode (disabled by default): if data later proves stable, allow a learned CPT risk ceiling only via explicit parameters (`cpt_tolerance_gbp` or `cpt_multiplier`) and mark source as empirical.

#### 8.2.2A Suppression reactivation target ladder (strict order)
When `buy_box_state IN {SUPPRESSED_ASIN, DISQUALIFIED_SELF_PRICE}` the system must use direct signals before probing.

Priority order:
1. `competitive_price_threshold`
2. `competitive_price`
3. `average_selling_price`
4. `FOEP`
5. controlled probing

Rules:
- direct signals always outrank probing
- the ladder defines an eligibility restoration target, not a profit target
- any chosen suppression target must clamp space and must not widen the optimiser range

#### 8.2.2B Suppression ceiling logic
Create:
- `suppression_ceiling_landed_temp`
- `suppression_ceiling_source`
- `suppression_ceiling_confidence`
- `suppression_ceiling_set_utc`
- `suppression_ceiling_expiry_utc`

Rules:
- derive from the highest-priority valid direct signal if one exists
- otherwise derive from a validated probe bracket
- must not widen price space
- must clamp optimisation space
- must expire after a defined interval

Mechanical rule:
```text
suppression_ceiling_landed_temp =
  min(
    suppression_reference_or_probe_ceiling,
    eligibility_ceiling_landed_signal?,
    eligibility_ceiling_landed_market?,
    compliance_ceiling_landed_gbp,
    demand_ceiling_landed_gbp?
...
#### 8.2.2C Threshold bracket discovery model
Maintain:
- `highest_eligible_price`
- `lowest_ineligible_price`
- `suppression_threshold_estimate`
...
Probing rules:
- bounded downward probing
- stop descent immediately when eligibility returns
- perform upward recon to determine ceiling

Guardrails:
- probing cannot fall below `anchor_floor_price`
- hard floor remains absolute
- structure drift, promo contamination, or writer conflict make the window inadmissible
...
Suppression never bypasses the three-ceiling model. It tightens the eligibility boundary inside it.
...
| `suppression_reactivation` | Suppression temp or Eligibility | You are restoring Featured Offer eligibility and preventing immediate re-suppression |
...
If suppression target or suppression ceiling implies pricing below `anchor_floor_price` or `hard_floor_price_gbp`:
- do not continue automated suppression discovery
...
- `suppression_reactivation` (eligibility restoration only)
- `pressure` (manual-only, capital strategy)

### 9.2 State selection rules (high level)
1) If a pressure case is `active` -> `state = pressure`
2) Else if `buy_box_state IN {SUPPRESSED_ASIN, DISQUALIFIED_SELF_PRICE}` and suppression evidence is confirmed -> `state = suppression_reactivation`
3) Else if competition absent -> `state = low_no_competition`
4) Else if volatility / integrity risk high -> `state = defensive_hold`
5) Else -> `state = maximize_profit`
6) `maximize_share` and `floor_discovery` require Supervisor approval and are time-boxed.

Rules:
- `LOST_TO_COMPETITOR` is not a suppression state
- `UNKNOWN` is not a suppression state
- `suppression_reactivation` is separate from pressure and separate from normal competition logic
- current Phase Engine behaviour still holds price when Buy Box is suppressed or unknown
- current Phase Engine does not perform suppression reactivation logic
- this v10 state is future canonical design and must not be claimed as already implemented

### 9.3 Lane vs State (Portfolio Governor integration)
- If `portfolio_gate_status = fail`:
  - states allowed: `low_no_competition`, `maximize_profit` (restricted), `defensive_hold`, `suppression_reactivation` (restricted)
...
- If `portfolio_gate_status = restricted`:
  ...
  - suppression reactivation allowed only with direct signals or bounded probing limits
- Pressure requires:
  - `portfolio_lane = fight`
  - `pressure_allowed_flag = true`
  - Head approval of a pressure case

Exit rule for suppression state:
- exit `suppression_reactivation` only when `buy_box_eligible_offers > 0` and `PRICING_HEALTH` is cleared
- after exit, keep the temporary suppression ceiling active through a cool-off window

---

## 10) Pressure State (Nuclear Mode) - Full Integration (Nuclear Mode) — Full Integration
...
- Buy Box eligible offer count
- channels and delivery postures
- new variants / exits
- coupons/promos
- offer count changes

### Step 3A - Buy Box State Classification (Mandatory)
Classify:
- `NORMAL`
- `LOST_TO_COMPETITOR`
- `SUPPRESSED_ASIN`
...
Rules:
- suppression is an eligibility state, not missing data
- `UNKNOWN` is hold-and-refresh only
- direct pricing-health evidence outranks inferred snapshot logic
...
- compute `suppression_ceiling_landed_temp`
- clamp feasible space to the tightest valid ceiling
- never widen space because of suppression logic
...
Rule:
- if suppression is active, this engine may inform current competition context but it must not update persistent delta memory from the suppression window
...
### Step 10A - Suppression Reactivation Flow (`suppression_reactivation` only)
1. Use the strict direct-signal ladder first:
   - `competitive_price_threshold`
   - `competitive_price`
   - `average_selling_price`
   - `FOEP`
2. If a direct signal exists, set the reactivation target and clamp with `suppression_ceiling_landed_temp`.
3. If direct signals do not exist, run bounded downward probing.
4. Stop descent immediately when eligibility returns.
5. Perform upward recon to determine the highest still-eligible ceiling.
6. Never probe below `anchor_floor_price`.
7. Exit only when `buy_box_eligible_offers > 0` and `PRICING_HEALTH` cleared.

### SUPPRESSION THRESHOLD INFERENCE
When direct suppression references do not exist, the runtime must still reduce the probe space before it starts stepping down.

Canonical inference rule:
- If `buy_box_state = SUPPRESSED_ASIN`
- and competitor offers exist
- and no offer currently has the Buy Box
- then set `suppression_threshold_upper_bound = lowest_competitor_price`

Why this rule is valid:
- If every visible offer is suppressed, the Featured Offer clearing point is already below the visible competitor cluster.
- Any price above the lowest visible competitor is already proven non-clearing for the listing as a whole.
- Starting from our current price in that case wastes probe space and delays recovery.

Search-space reduction logic:
- The suppression discovery interval becomes:
  - upper bound = `lowest_competitor_price`
  - lower bound = `max(anchor_floor_price, hard_floor_price_gbp)`
- The runtime must begin suppression discovery from that inferred upper bound instead of from our current price when our current price is above it.
- If our current price is already inside the reduced interval, probing continues from the current in-range price and does not widen upward.

Interaction with suppression probing:
- Direct signals still outrank inference.
- The priority order is:
  - direct suppression target
  - validated suppression threshold memory / bracket
  - inferred upper bound from lowest competitor under all-offers-suppressed conditions
  - bounded downward step inside the remaining interval
- The inference only changes the probe start bound. It does not change the success criteria, stop-on-return rule, or upward recon rule.

Interaction with suppression ceiling model:
- The inferred upper bound may seed `suppression_ceiling_landed_temp` as a temporary eligibility clamp.
- That temporary clamp must still never widen the existing feasible space.
- The clamp remains temporary and must still expire and revalidate under the normal suppression ceiling rules.

Guardrails:
- Hard floor remains absolute.
- `anchor_floor_price` remains the suppression probe floor when it is above hard floor.
- The inferred upper bound must still clamp through the existing suppression ceiling and final ceiling chain before any write is proposed.
- Learning isolation remains unchanged: suppression windows still cannot update competitor delta, aggressor, or demand learning.
- Existing write verification, writer-mode locks, and observability gates remain unchanged.

Non-effect rule:
- This inference applies to the all-offers-suppressed `SUPPRESSED_ASIN` case only.
- It must not change `NORMAL`, `LOST_TO_COMPETITOR`, or `DISQUALIFIED_SELF_PRICE` behaviour unless a shared helper is required and still preserves the same state-specific rules.
...
- suppression probing cannot fall below `anchor_floor_price`

### Step 13 - Approval + Execution
- normal state: Supervisor approves the move, Executioner writes
- suppression reactivation state: Executioner writes only the approved reactivation action or bounded probe action
...
Suppression-specific rule:
- suppression windows must not update competitor delta learning, aggressor classification, or demand elasticity models
- only the dedicated suppression threshold memory channel may update when the suppression window itself is admissible

---

## 12) Portfolio Governor (Mandatory Daily Capital Allocation Gate) (Mandatory Daily Capital Allocation Gate)
...
- and any update to persistent memory (delta, behaviour classifiers, delivery multiplier, demand ceiling, suppression threshold memory).
...
- `buy_box_state`
- suppression/pricing-health posture flags
...
- Suppression threshold memory updates
...
- key flags: suppression/pricing-health/eligibility-change, stock status, promo suspicion, writer mode

**Output (minimum):**
- `admissible_flag` (boolean; true iff `context_quality_score > 0`)
- `context_quality_score in [0,1]`
...
2) **Buy Box suppressed** at any point in the window for general competitor/demand learning.
...
7) **Writer conflict** during the window.

Exception:
- suppression windows remain a hard fail for general learning, but may still update the dedicated suppression threshold memory channel if the suppression-specific window is otherwise clean and explicitly processed through the suppression-learning contract.
...
4) **Suppression-state ambiguity:**
   - conflicting evidence between direct event signals and snapshot state.
...
- Minor competitor drift -> x 0.7
- Moderate drift -> x 0.4
- Delivery posture shift (minor) -> x 0.7
- Buy Box rotation noise detected -> x 0.6
- suppression-state ambiguity -> x 0.5
...
- **Suppression threshold memory** (updated only in the dedicated suppression channel)
...
- `highest_eligible_price` (suppression bracket bound; suppression-only)
- `lowest_ineligible_price` (suppression bracket bound; suppression-only)
- `suppression_threshold_estimate` (soft suppression estimate)
...
- Seller/variant memory updates only if:
  ...
  - no suppression was active in the learning window
...
- Demand model updates only if:
  ...
  - no suppression was active in the learning window
- Suppression threshold memory updates only if:
  - `buy_box_state IN {SUPPRESSED_ASIN, DISQUALIFIED_SELF_PRICE}`
  - direct signal or clean probe evidence exists
  - market structure stayed stable
  - promo contamination absent
  - writer mode stable
...
Mandatory suppression memory fields:
- `highest_eligible_price`
- `lowest_ineligible_price`
- `suppression_threshold_estimate`
- `suppression_threshold_confidence`
...
- If `suppression_threshold_confidence < min_suppression_confidence_to_trust` then:
  - suppression memory may seed a starting bracket only
  - it must not widen space
  - direct signals still outrank it
...
- If `days_since_suppression_last_validated > suppression_stale_days_threshold` then reduce suppression confidence and expire `suppression_ceiling_landed_temp` unless revalidated.

### 14.7 Seller Delta Learning Loop Protocol (Variant-Level, Effective-Price Space)
Seller delta learning remains variant-level and effective-price based.

Rule:
- suppression windows must not update delta memory
- suppression is a dedicated parallel learning channel, not a special case inside delta learning

### 14.8 Protection Against False Aggressor Detection (must use OAS)
Aggressor classification must not consume suppression windows as evidence of competitor aggression.

If suppression is active:
- block aggressor classifier updates
- block persistence-score updates that would treat Amazon eligibility behaviour as competitor behaviour
...
- ceiling facts (compliance/eligibility/suppression/demand/final, binding ceiling)
- eligibility facts (FOEP/CPT/competitive-price/average-selling-price used, confidence)
- Buy Box state facts (`buy_box_state`, eligible-offer count, pricing-health state)
...
Suppression executions additionally must log:
- `suppression_case_id`
...
- `lowest_ineligible_price`
- `suppression_threshold_estimate`
- whether descent stopped due to eligibility return
- whether upward recon ran
- whether `anchor_floor_price` blocked further probing
- whether exit conditions were satisfied
...
This section defines the single live truth for suppressed SKUs from runtime decision to writer result to dashboard display.

### 15A.1 Canonical separation
- `write_capability`
  - The SKU is allowed through scope and gate checks.
  - Capability is not success.
- `write_attempt`
  - Runtime proposed a real action for the SKU.
  - Blocked outcomes still count as attempted action.
- `write_applied`
  - Writer verification confirmed the action landed.
  - This is the only write-success state.
- `suppression_active`
  - Buy Box truth is still suppressed or self-disqualified, or suppression memory remains the latest authoritative state.
- `suppression_resolved`
  - Later runtime truth confirms suppression is no longer active.
  - Resolution may not be inferred from capability or a generic ceiling.
- `displayed_ceiling_field`
  - Analyst-facing model ceiling that may remain visible for context.
- `true_binding_ceiling`
  - The ceiling that actually constrains live action for the SKU right now.

### 15A.2 Ceiling rule for suppression
- Generic compliance ceiling may remain visible as a model field.
- When suppression reactivation is active, `suppression_ceiling_landed_temp` is the true active ceiling.
- Dashboard and status truth must treat the suppression clamp as the binding ceiling even if a higher generic compliance ceiling also exists.

### 15A.3 Dashboard and status rules
- Status colour/state must follow actual suppression truth and actual writer outcome, not scope eligibility alone.
- `ACTIVE_WRITE`, `write_effective=1`, or equivalent capability flags must never by themselves create a healthy-success presentation for a suppressed SKU.
- A suppressed SKU must remain visibly unresolved when any of the following are true:
  - suppression is still active
  - the latest writer outcome is blocked, not applied, observability-only, or read-only
  - Buy Box truth is still suppressed or unresolved
- `WRITE_NOT_APPLIED`, `OBSERVABILITY_BLOCK_NO_WRITE`, `READ_ONLY_NO_WRITE`, and no-attempt states must not be displayed as success.
- A suppressed SKU may only return to a healthy presentation after runtime truth confirms suppression resolved and no unresolved writer outcome still governs the SKU.

### 15A.4 Required dashboard truth fields
- Dashboard/status must distinguish:
  - write capability
  - actual strategy state
  - actual writer outcome
  - actual observed live price
  - actual Buy Box truth
  - true binding ceiling
  - optional generic model ceiling
- The generic model ceiling may remain visible, but it must not replace the true binding ceiling in suppression truth.

---

## 15B) CANNOT-COMPETE EXECUTION MODEL

This section defines the canonical live rule for SKUs that have already crossed from normal competition into exit behaviour.

### 15B.1 Canonical minimum executable target
- The runtime must use `active_floor_gbp` as the minimum executable target for cannot-compete execution.
- `active_floor_gbp` resolves in this order:
  1. explicit phase exit floor, when one exists
  2. current runtime hard floor
- This removes ambiguity between `hard_floor`, `phase floor`, `controlled exit target`, and `minimum executable price`.
- In Phase 3 or Phase 4, if no explicit exit floor is persisted yet, the live runtime must fall back to the current hard floor rather than holding above it.

### 15B.2 Phase-owned execution states
- Phase 2: `MARGIN_COMPRESS_TO_FLOOR`
- Phase 3: `CONTROLLED_EXIT_TO_FLOOR`
- Phase 4: `LIQUIDATE_TO_FLOOR`
- These are execution states, not just analytics labels.
- When a SKU is in a cannot-compete phase and normal rival-led logic would otherwise hold, runtime must switch to the phase-owned floor-seeking state.

### 15B.3 Execution rule
- A cannot-compete SKU must move toward `active_floor_gbp` using the existing downward guardrails:
  - `max_step_down_gbp`
  - `max_daily_drop_gbp`
- The target for the current cycle is the lowest permitted price toward `active_floor_gbp` under those guards.
- If the SKU is already at or below `active_floor_gbp`, no write is required.
- Hard floor, anchor floor, suppression clamp, and existing write guardrails remain intact.

### 15B.4 Daily-intel degraded fallback
- Missing or stale daily intel normally forces defensive hold.
- Exception: when a SKU is already in cannot-compete execution state and the action is a downward move toward `active_floor_gbp`, runtime may continue in degraded mode.
- This degraded path is allowed only for downward floor-seeking and must stay reason-coded.
- Capability, write attempt, write applied, and blocked outcomes must still be reported separately.

### 15B.5 Display and runtime truth
- Dashboard/runtime must show the actual cannot-compete execution state.
- A floor-seeking write attempt that is blocked must remain blocked in status truth.
- A successful floor-seeking write may be shown as applied, but the strategy state must still show the active phase execution state.
...
- build canonical `buy_box_state` classification
- persist direct suppression reference inputs (`competitive_price_threshold`, `competitive_price`, `average_selling_price`, `FOEP`)

### Stage G - Suppression classification + temporary ceiling
- activate suppression detection rules
- create `STATE_SUPPRESSION_REACTIVATION`
- create `suppression_ceiling_landed_temp`
- hold current Phase Engine behaviour as implementation reality until suppression flow is actually built

### Stage H - Direct-signal suppression reactivation
- implement strict direct-signal ladder
- ensure direct signals always outrank probing
- add cool-off clamp after reactivation exit

### Stage I - Bounded suppression discovery + dedicated learning channel
- add `highest_eligible_price`
- add `lowest_ineligible_price`
- add `suppression_threshold_estimate`
- add `suppression_threshold_confidence`
- add `suppression_last_validated_utc`
- add bounded downward probing, stop-on-return, upward recon, and suppression-only learning updates

### Stage J - Demand ceiling learning (slow)
- enable demand ceiling model in effective-price space with strict stability gating

### Stage K - DVE learned correction (multiplier)
- enable multiplier learning with strict evidence rules

### Stage L - Event-led runtime hardening (pre-expansion, mandatory)
...
- suppression-specific event orchestration and health checks
...
- suppression windows isolated correctly from competitor and demand learning
- temporary suppression ceilings expire and revalidate correctly
...
## 18) Runtime Hardening Rules (Mandatory)

This section is part of the canonical plan. It groups the hard runtime requirements (event-led scheduling, rate limits/budgets, write contracts, suppression handling, and expansion gates).
...
- `PRICING_HEALTH`
  ...
  - may route the SKU into suppression classification / suppression reactivation logic
  - hard-fails general learning windows in OAS (`OAS_HARD_FAIL_PRICING_HEALTH_EVENT`)
- `ANY_OFFER_CHANGED`
...
### 18.4 Eligibility + Suppression Fallback Contract
...
Suppression handling obeys the same principle: if direct suppression signals disappear, the system must degrade safely and must not widen price space.
...
Suppressed SKUs must additionally have either:
- a direct-signal reactivation target, or
- a bounded probe state, or
- a hold-and-manual-review outcome

#### 18.4.2 FOEP and suppression failure modes must be reason-coded
You must explicitly handle:
- FOEP missing
- `ASIN_NOT_ELIGIBLE`
- FOEP sanity check fail
- FOEP stale beyond threshold
- suppression direct signal missing
- suppression direct signal conflicts with probe bracket
- suppression target below `anchor_floor_price`

Missing/invalid signals must:
- lower confidence
- switch to a safer fallback
- never widen price space

#### 18.4.3 Eligibility and suppression health checks (required)
...
- alert if suppression is active with no valid suppression target state
- alert if `suppression_ceiling_landed_temp` is expired but still binding

### 18.5 Price Write Contract (no silent writes)
No price write is allowed without:
- reason code set
- driving game context (variant/seller pressure, suppression reactivation, or normal optimization context)
- floor/ceiling clamp notes
- confidence score and expiry timestamp
- action type flag (`probe` vs `normal` vs `suppression_reactivation`)
...
- suppression reactivation: eligibility restoration with minimum necessary price movement
...
- suppression windows leaking into delta learning (must be zero)
- suppression windows leaking into aggressor updates (must be zero)
- suppression windows leaking into demand learning (must be zero)

### 18.9 Expansion Gate Upgrade (supersedes Stage L minimum)
...
- suppression classification is stable
- suppression ceilings expire and revalidate correctly
- no suppression-learning leakage into non-suppression models
...
# Final Assessment After v10 Integration
...
* Buy Box suppression is treated as eligibility, not as missing data.
* Suppression recovery is isolated from competitor and demand learning.
* Identity is structurally continuous.
* Writer conflicts are eliminated.

At that point, the plan is no longer just intelligent.

It becomes portfolio-grade, scale-safe, and suppression-aware.
