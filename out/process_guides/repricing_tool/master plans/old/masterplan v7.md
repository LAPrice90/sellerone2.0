# Masterplan v7 — Unified Repricing Intelligence + Delivery Value Engine + Three-Ceiling Governance + Pressure State + Dual-Identity Offer Model + Outcome Admissibility Learning Loop

Status: Active Draft (Unified v7)  
Date: 2026-02-12  
Owner: Luke (business intent), Codex (execution support)

## 0) What Changed vs v6.1 (diff guide)
1. **Outcome Admissibility is now a system-wide Truth Filter (not just a delta add-on)**
   - One shared admissibility decision gates **all learning updates** (seller delta, DVE multiplier, demand learning) and **all behaviour classifiers** (aggressor / persistence / floor inference).
   - Contaminated outcomes can no longer silently poison memory.
2. **Outcome Admissibility Service (OAS) is formally specified**
   - Standard invariants, `context_quality_score ∈ [0,1]`, and required reason codes.
   - Mandatory start/end `market_structure_hash` on every probe window and learning window.
3. **Seller Delta Learning Loop is now fully specified (protocol + confidence)**
   - Bracket logic (first win/loss → narrow) remains, but updates are **confidence- and quality-weighted**.
   - Delta memory now stores valid/contaminated test counts and last valid timestamp for confidence decay.
4. **Learning Consumer Contract is now explicit**
   - Optimiser must refuse to treat low-confidence deltas as truth; it must widen uncertainty and/or fall back to safe heuristics and trigger re-learning.
5. **Logging is upgraded to make contamination visible**
   - Probe/event logs must include start/end hashes, admissibility decision, quality score, and contamination reasons.
   - Any learning update without a passing admissibility record is a hard bug (must be zero).
...
Blindly "weight-updating" `highest_delta_win` / `lowest_delta_loss` can corrupt the bracket. v7 separates:
...
v7 adds one rule: **the loop only learns from admissible outcomes (OAS).**
...
## 18) v7 Runtime Hardening Rules (Mandatory)
