# Masterplan v5 — Unified Repricing Intelligence + Delivery Value Engine + Three-Ceiling Governance + Pressure State

Status: Active Draft (Unified v5)  
Date: 2026-02-12  
Owner: Luke (business intent), Codex (execution support)

## 0) What Changed vs v4 (diff guide)
...
Build ladder between `hard_floor` and `final_ceiling`.

### Step 10 — Outcome + Profit Estimation (Normal states only)
Estimate expected units/day and profit/day for each candidate.
Choose argmax expected profit/day (default).
