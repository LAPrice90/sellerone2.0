# Masterplan v6.1 — Unified Repricing Intelligence + Delivery Value Engine + Three-Ceiling Governance + Pressure State + Dual-Identity Offer Model

Status: Active Draft (Unified v6.1)  
Date: 2026-02-12  
Owner: Luke (business intent), Codex (execution support)

## 0) What Changed vs v6 (diff guide)
1. **Outcome Admissibility Layer is now mandatory for learning updates**
   - Delta updates require context invariants (competitor stability, no suppression, no stockout, no major delivery shift, no pricing health/eligibility event).
   - Low-quality outcomes are down-weighted using `context_quality_score` instead of blindly updating memory.
2. **Event-led runtime is now a hard architecture rule**
   - `ANY_OFFER_CHANGED` drives tactical refresh targeting.
   - `PRICING_HEALTH` drives risk override and safe-mode behavior.
   - Polling remains fallback only.
3. **Rate-limit control is now explicit**
   - Global API safety threshold plus per-SKU refresh budgets are mandatory.
   - One volatile SKU must not consume the portfolio's budget.
4. **Write accountability tightened**
   - No price write without complete reason payload, confidence, expiry, and probe-vs-normal flag.
5. **Eligibility fallback formalized**
   - FOEP-unavailable/`ASIN_NOT_ELIGIBLE` paths must be explicit and reason-coded.
6. **Lane north-star metrics added**
   - Managed lanes optimize contribution profit/day with volatility penalty.
   - Passive lane prioritizes margin protection and eligibility safety.
7. **All v6 content remains intact**
   - v6.1 is additive hardening, not a replacement of v6 logic.
8. **Dual-Identity Offer Model is now mandatory (structural continuity)**
...
9. **Offer Instance Layer is formally split into Fact + Dimension**
   - Offer Snapshot Fact (event-level truth, no dedupe)
   - Offer Variant Dimension (structural identity)
10. **Market-structure stability checks are tightened**
...
11. **Logging and outputs updated**
...
## 18) v6.1 Addendum - Mandatory Updates (Additive)

This section is additive. All v6 sections above remain valid unless this addendum sets a stricter rule.

### 18.1 Outcome Admissibility Layer (required before learning update)
Do not update seller/variant learning memory unless the outcome passes admissibility checks.

Required context invariants:
- competitor set stability within test window
  - `offer_count_delta <= allowed_threshold`
  - no new seller-of-interest appears
  - no core competitor exits unexpectedly
- no listing suppression during test window
- no self stockout during test window
- no major delivery posture shifts (us or key rivals)
- no pricing health or eligibility-status event during test window

Update policy:
- if invariants fail hard -> record outcome, block update
- if invariants are partially degraded -> apply weighted update using `context_quality_score` in [0,1]
- if invariants pass -> normal update

### 18.2 Event-Led Runtime Contract (required for scale)
Scheduler rule:
- event-led by default
- polling fallback only

Notification roles:
- `ANY_OFFER_CHANGED` -> tactical refresh targeting
- `PRICING_HEALTH` -> risk override (`defensive_hold` or `parked`) + safe clamp behavior

Storm control:
- per-SKU refresh cooldown
- dedupe events by `(sku, event_type, short_time_window)`
- batch refresh where safe to avoid API stampedes

### 18.3 API Budget and Rate-Limit Controls (required)
Budget layers:
- global budget safety threshold (hard stop/protective throttling)
- per-SKU rolling budget based on lane and volatility

Priority under constraint:
- `PRICING_HEALTH` actions are higher priority than routine market refresh
- high-value lanes protected from starvation by noisy SKUs

### 18.4 Eligibility Fallback Contract (FOEP gaps)
When FOEP is unavailable (including `ASIN_NOT_ELIGIBLE`):
- use CPT and market-based eligibility signals when available
- then fallback to safe historical proxy if required
- never hide fallback path; must be reason-coded in logs
...
### 18.7 v6.1 Additional Outputs (minimum)
These outputs are required in addition to section 15:
- `out/h_outcome_admissibility_log.csv`
- `out/h_notification_event_log.csv`
- `out/h_api_budget_usage_by_sku.csv`
- `out/h_api_budget_usage_total.csv`
- `out/h_rate_limit_health.csv`

### 18.8 v6.1 Additional Health Checks
