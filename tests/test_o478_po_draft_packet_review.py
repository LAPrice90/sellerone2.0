...
def _design_line(
    *,
    row_id: str,
    sku: str,
    design_state: str = "ready_for_local_po_line_design_review_only",
    qty: str = "2",
    cost: str = "3",
    line_value: str = "6",
    ...
    return {
        ...
        "barcode": "123",
        "designed_order_qty": qty,
        "designed_unit_cost_gbp": cost,
        "designed_line_value_gbp": line_value,
        "source_po_draft_readiness_state": "ready_for_local_po_draft_review_only",
        "line_design_state": design_state,
        "line_design_block_reasons": "" if design_state == "ready_for_local_po_line_design_review_only" else "source_po_draft_not_ready",
...
def test_o478_builds_local_po_draft_packet_review(tmp_path: Path) -> None:
    _write_contract_rows(
        tmp_path,
        "restock_po_line_design_preview_lines_live",
        [_design_line(row_id="row-ready", sku="SKU-READY")],
    )

    lines_df, summary_df, health_df = build_po_draft_packet_review(
        root=tmp_path,
        review_utc=OBSERVED,
        refresh_design=False,
    )

    line = lines_df.iloc[0]
    assert line["packet_review_line_state"] == "ready_for_local_po_draft_packet_review_only"
    assert line["review_order_qty"] == "2"
    assert line["review_unit_cost_gbp"] == "3"
    assert line["review_line_value_gbp"] == "6"
    ...
    assert summary_df.iloc[0]["packet_review_state"] == "ready_for_local_po_draft_packet_review_only"
    assert summary_df.iloc[0]["po_creation_allowed"] == "0"
    assert set(health_df["status"].tolist()) == {"ok"}

    written = read_o_contract_df(tmp_path, "restock_po_draft_packet_review_lines_live")
    assert len(written.index) == 1


def test_o478_blocks_missing_qty_and_cost_even_after_line_design(tmp_path: Path) -> None:
    _write_contract_rows(
        tmp_path,
        "restock_po_line_design_preview_lines_live",
        [_design_line(row_id="row-missing", sku="SKU-MISSING", qty="", cost="", line_value="")],
    )

    lines_df, _summary_df, health_df = build_po_draft_packet_review(
        root=tmp_path,
        review_utc=OBSERVED,
        refresh_design=False,
    )
    ...
    line = lines_df.iloc[0]
    assert line["packet_review_line_state"] == "blocked_from_local_po_draft_packet_review"
    assert "missing_or_invalid_review_qty" in line["packet_review_block_reasons"]
    assert "missing_or_invalid_unit_cost" in line["packet_review_block_reasons"]
    assert set(health_df["status"].tolist()) == {"ok"}


def test_o478_blocks_source_action_flag_without_writing_po_path(tmp_path: Path) -> None:
    _write_contract_rows(
        tmp_path,
        "restock_po_line_design_preview_lines_live",
        [_design_line(row_id="row-unsafe", sku="SKU-UNSAFE", po_file_write_allowed="1")],
    )
    ...
    line = lines_df.iloc[0]
    assert line["packet_review_line_state"] == "blocked_from_local_po_draft_packet_review"
    assert "source_po_file_write_allowed_not_zero" in line["packet_review_block_reasons"]
