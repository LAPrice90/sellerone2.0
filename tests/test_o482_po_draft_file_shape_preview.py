...
def _hold_line(
    *,
    row_id: str,
    sku: str,
    hold_state: str = "held_for_local_po_draft_review_only",
    qty: str = "2",
    cost: str = "3",
    line_value: str = "6",
    ...
    return {
        ...
        "barcode": "123",
        "hold_order_qty": qty,
        "hold_unit_cost_gbp": cost,
        "hold_line_value_gbp": line_value,
        ...
        "hold_review_line_state": hold_state,
        "hold_review_reasons": "local_review_hold_zero_action" if hold_state == "held_for_local_po_draft_review_only" else "source_packet_review_not_ready",
...
def test_o482_builds_local_po_draft_file_shape_preview(tmp_path: Path) -> None:
    _write_contract_rows(
        tmp_path,
        "restock_po_draft_hold_review_lines_live",
        [_hold_line(row_id="row-ready", sku="SKU-READY")],
    )

    lines_df, summary_df, health_df = build_po_draft_file_shape_preview(
        root=tmp_path,
        shape_utc=OBSERVED,
        refresh_hold_review=False,
    )

    line = lines_df.iloc[0]
    assert line["file_shape_line_state"] == "ready_for_local_po_draft_file_shape_review_only"
    assert line["file_shape_block_reasons"] == ""
    assert line["file_shape_qty"] == "2"
    assert line["file_shape_unit_cost_gbp"] == "3"
    assert line["file_shape_line_value_gbp"] == "6"
    ...
    assert summary_df.iloc[0]["file_shape_state"] == "ready_for_local_po_draft_file_shape_review_only"
    assert summary_df.iloc[0]["po_creation_allowed"] == "0"
    assert set(health_df["status"].tolist()) == {"ok"}

    written = read_o_contract_df(tmp_path, "restock_po_draft_file_shape_preview_lines_live")
    assert len(written.index) == 1


def test_o482_blocks_missing_qty_and_cost_even_after_hold_review(tmp_path: Path) -> None:
    _write_contract_rows(
        tmp_path,
        "restock_po_draft_hold_review_lines_live",
        [_hold_line(row_id="row-missing", sku="SKU-MISSING", qty="", cost="", line_value="")],
    )

    lines_df, _summary_df, health_df = build_po_draft_file_shape_preview(
        root=tmp_path,
        shape_utc=OBSERVED,
        refresh_hold_review=False,
    )
    ...
    line = lines_df.iloc[0]
    assert line["file_shape_line_state"] == "blocked_from_local_po_draft_file_shape_review"
    assert "missing_or_invalid_file_shape_qty" in line["file_shape_block_reasons"]
    assert "missing_or_invalid_unit_cost" in line["file_shape_block_reasons"]
    assert set(health_df["status"].tolist()) == {"ok"}


def test_o482_blocks_source_action_flag_without_writing_po_path(tmp_path: Path) -> None:
    _write_contract_rows(
        tmp_path,
        "restock_po_draft_hold_review_lines_live",
        [_hold_line(row_id="row-unsafe", sku="SKU-UNSAFE", po_file_write_allowed="1")],
    )
    ...
    line = lines_df.iloc[0]
    assert line["file_shape_line_state"] == "blocked_from_local_po_draft_file_shape_review"
    assert "source_po_file_write_allowed_not_zero" in line["file_shape_block_reasons"]
