...
def main() -> None:
    ...
    try:
        ...
        if USE_API_OWNER:
            env = os.environ.copy()
            env["API_COLLECTION_DATASETS"] = "inventory_inbound"
            proc = subprocess.run(
                [sys.executable, str(ROOT / "run_api_collection.py")],
                env=env,
                capture_output=True,
                text=True,
            )
            ...
            if proc.returncode != 0:
                raise RuntimeError(
                    f"API owner inventory collection failed rc={proc.returncode}: "
                    f"{(proc.stdout or '').strip()} {(proc.stderr or '').strip()}".strip()
                )
