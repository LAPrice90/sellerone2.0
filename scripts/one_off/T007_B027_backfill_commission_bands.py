...
def main() -> None:
    ...
    # Filter UK only when marketplace_id present
    ...
    df = df[df["__price_total"].notna() & df["__comm_ex"].notna()]
    if df.empty:
        print({"status": "skip", "reason": "missing_price_or_commission"})
        return

    df["__unit_price"] = (df["__price_total"].abs() / df["__qty"]).round(2)
    df["__comm_pct"] = (df["__comm_ex"].abs() / df["__price_total"].abs() * 100.0).round(2)
    df = df[df["__comm_pct"] > 0]

    bands = {
        "last_commission_pct_10": df[df["__unit_price"] <= 10.0],
        "last_commission_pct_100": df[df["__unit_price"] > 10.0],
    }
    ...
    for col in ["last_commission_pct_10", "last_commission_pct_100", "last_commission_updated", "last_commission_source"]:
        if col not in idx:
            idx[col] = len(headers)
            headers.append(col)
            for r in rows[1:]:
    ...
    for field, bdf in bands.items():
        ...
        for sku, grp in bdf.groupby("SKU"):
            if sku not in sku_rows:
                continue
            window = grp.tail(10)
            counts = window["__comm_pct"].value_counts()
            if counts.empty:
                continue
            top = float(counts.index[0])
            top_count = int(counts.iloc[0])
            recent_dt = window[window["__comm_pct"] == top]["__date"].max()
            pct_str = round_pct(top)
            dt_str = recent_dt.strftime("%Y-%m-%dT%H:%M:%SZ") if pd.notna(recent_dt) else ""
            row = rows[sku_rows[sku]]
            row[idx[field]] = pct_str
            row[idx["last_commission_updated"]] = dt_str
            row[idx["last_commission_source"]] = "Level3"
