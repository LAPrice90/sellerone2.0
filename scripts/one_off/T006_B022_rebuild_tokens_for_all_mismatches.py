...
def main() -> None:
    if not MISMATCH_PATH.exists() or not RECON_PATH.exists():
        print({"status": "skip", "reason": "missing_mismatch_or_recon"})
        return
    ...
    if recon.empty:
        print({"status": "skip", "reason": "empty_recon_file"})
        return
        ...
        summary_rows.append(
            {
                ...
                "status": "ok" if created == target_units else "partial",
                "updated_at": now_iso,
            }
        )
        affected.add(sku)
