"""
Fetch monthly FBA storage fee report (GET_FBA_STORAGE_FEE_CHARGES_DATA) and write CSV.

Defaults:
- Local output only (no sheets).
- Skips unless day >= 10 or STORAGE_FEE_FORCE=1.
- Skips if output for the month already exists unless STORAGE_FEE_FORCE=1.
"""
...
OUT_CSV = Path("out/fba_storage_fee_charges_monthly.csv")
...
def _month_window() -> Tuple[str, str, str]:
    override_year = os.environ.get("STORAGE_FEE_OVERRIDE_YEAR", "").strip()
    override_month = os.environ.get("STORAGE_FEE_OVERRIDE_MONTH", "").strip()
...
def main() -> None:
    ...
    force = os.environ.get("STORAGE_FEE_FORCE", "0").strip() == "1"
    day = datetime.now(timezone.utc).day
    if day < 10 and not force:
