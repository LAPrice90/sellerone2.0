...
OUT_MISSING = Path("out/inbound_missing_units.csv")
...
def main() -> None:
    ...
    if contents.empty and ledger.empty:
        ...
        pd.DataFrame().to_csv(OUT_MISSING, index=False)
        print({"status": "success", "rows": 0, "snapshot": str(OUT_MISSING)})
        return
    ...
    missing = merged[merged["missing_qty"] > 0].copy()
    OUT_MISSING.parent.mkdir(parents=True, exist_ok=True)
    missing.to_csv(OUT_MISSING, index=False)
    print({"status": "success", "rows": len(missing), "snapshot": str(OUT_MISSING)})
