...
REVIEW_ID_PREFIX = "o_po_draft_packet_review_v1"
ZERO_FLAG_COLUMNS = (
...
def _review_id(row: pd.Series) -> str:
    ...
    return f"{REVIEW_ID_PREFIX}:{_safe_fragment(basis)}"


def _source_flags_are_zero(row: pd.Series) -> bool:
    return all(_normalize_text(row.get(column, "")) == "0" for column in ZERO_FLAG_COLUMNS)


def _line_state(row: pd.Series) -> tuple[str, str]:
    reasons: list[str] = []
    if _normalize_text(row.get("po_line_design_packet_id", "")) == "":
        reasons.append("missing_line_design_packet_id")
    if _normalize_text(row.get("source_class", "")) not in SOURCE_CLASSES:
        reasons.append("unsupported_source_class")
    if _normalize_text(row.get("line_design_state", "")) != SOURCE_READY_STATE:
        reasons.append("source_line_design_not_ready")
    ...
    if _positive_num(row.get("designed_order_qty", "")) is None:
        reasons.append("missing_or_invalid_review_qty")
    if _positive_num(row.get("designed_unit_cost_gbp", "")) is None:
        reasons.append("missing_or_invalid_unit_cost")
    if _positive_num(row.get("designed_line_value_gbp", "")) is None:
...
def _build_lines(review_utc: str, design_df: pd.DataFrame) -> pd.DataFrame:
    if design_df.empty:
        return pd.DataFrame()
    rows: list[dict[str, str]] = []
    for _, line in design_df.iterrows():
        review_id = _review_id(line)
        state, reasons = _line_state(line)
        output_row = {
            "review_utc": review_utc,
            "po_draft_packet_review_id": review_id,
            "po_line_design_id": _normalize_text(line.get("po_line_design_id", "")),
            ...
            "source_design_utc": _normalize_text(line.get("preview_utc", "")),
            ...
            "review_order_qty": _normalize_text(line.get("designed_order_qty", "")),
            "review_unit_cost_gbp": _normalize_text(line.get("designed_unit_cost_gbp", "")),
            "review_line_value_gbp": _normalize_text(line.get("designed_line_value_gbp", "")),
            "source_line_design_state": _normalize_text(line.get("line_design_state", "")),
            "packet_review_line_state": state,
            "packet_review_block_reasons": reasons,
            ...
            "packet_review_basis": "local_po_draft_packet_review_only",
        }
...
def _build_summary(review_utc: str, lines_df: pd.DataFrame) -> pd.DataFrame:
    if lines_df.empty:
        return pd.DataFrame()
    rows: list[dict[str, str]] = []
    grouped = lines_df.groupby(
        ["po_draft_packet_review_id", "po_line_design_packet_id", "approval_packet_id", "supplier_name", "supplier_code"],
        dropna=False,
        sort=True,
    )
    for (review_id, design_packet_id, approval_packet_id, supplier_name, supplier_code), group in grouped:
        ready_count = int(group.get("packet_review_line_state", pd.Series(dtype=str)).map(_normalize_text).eq(READY_STATE).sum())
        blocked_count = int(group.get("packet_review_line_state", pd.Series(dtype=str)).map(_normalize_text).eq(BLOCKED_STATE).sum())
        qty_total = pd.to_numeric(group.get("review_order_qty", ""), errors="coerce").fillna(0).sum()
        value_total = pd.to_numeric(group.get("review_line_value_gbp", ""), errors="coerce").fillna(0).sum()
        reason_counter: Counter[str] = Counter()
        for reason_text in group.get("packet_review_block_reasons", pd.Series(dtype=str)).tolist():
        ...
        row = {
            "review_utc": review_utc,
            "po_draft_packet_review_id": _normalize_text(review_id),
            ...
            "review_order_qty_total": _num_text(float(qty_total)),
            "review_order_value_gbp": _num_text(float(value_total)) if value_total > 0 else "",
            "packet_review_state": packet_state,
            "packet_review_block_reasons": "|".join(reason for reason, _count in reason_counter.most_common(8)),
...
def _build_health(review_utc: str, lines_df: pd.DataFrame, summary_df: pd.DataFrame, source_paths: list[Path]) -> pd.DataFrame:
    ...
    if not lines_df.empty:
        for _, row in lines_df.iterrows():
            ...
            if _contains_unsafe_language(row.get("packet_review_line_state", ""), row.get("packet_review_block_reasons", "")):
                live_language_rows.append(row_label)
            if _normalize_text(row.get("packet_review_line_state", "")) == READY_STATE and (
                _normalize_text(row.get("po_line_design_packet_id", "")) == ""
                or _normalize_text(row.get("source_line_design_state", "")) != SOURCE_READY_STATE
                or any(_normalize_text(row.get(column, "")) != "0" for column in SOURCE_FLAG_MAP.values())
                or _positive_num(row.get("review_order_qty", "")) is None
                or _positive_num(row.get("review_unit_cost_gbp", "")) is None
                or _positive_num(row.get("review_line_value_gbp", "")) is None
                or _normalize_text(row.get("source_class", "")) not in SOURCE_CLASSES
            ):
                false_ready_rows.append(row_label)
            if _normalize_text(row.get("packet_review_line_state", "")) == BLOCKED_STATE and _normalize_text(row.get("packet_review_block_reasons", "")) == "":
                missing_block_reason_rows.append(row_label)
    if not summary_df.empty:
        for _, row in summary_df.iterrows():
            review_id = _normalize_text(row.get("po_draft_packet_review_id", "")) or "missing_packet"
            if any(_normalize_text(row.get(column, "")) != "0" for column in ZERO_FLAG_COLUMNS):
                bad_summary_rows.append(f"{review_id}:action_flag")
            if _contains_unsafe_language(row.get("packet_review_state", ""), row.get("packet_review_block_reasons", "")):
                bad_summary_rows.append(f"{review_id}:unsafe_language")
    checks = [
        (
            "source_label_guard",
            not invalid_source_rows,
            f"invalid_source_rows={len(invalid_source_rows)};line_rows={len(lines_df.index)}",
            "Every PO draft packet review row must keep an approved O source class.",
        ),
        (
            "local_only_guard",
            ...
            "PO draft packet review must not write PO files, create POs, commit buying, receive stock, or send to Amazon.",
        ),
        (
            "packet_claim_guard",
            ...
            "A packet line can only be review-ready after safe line-design proof, quantity, unit cost, and line value.",
        ),
        (
            ...
            "PO draft packet review summary must not invent packets beyond review lines.",
        ),
    ]
...
def build_po_draft_packet_review(
    root: Path | None = None,
    *,
    review_utc: str | None = None,
    write_outputs: bool = True,
    refresh_design: bool = True,
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    root_path = Path(root) if root is not None else get_o_path_contract().root
    paths = ensure_o_directories(root=root_path)
    observed = review_utc or _utc_now_iso()
    if refresh_design:
        build_po_line_design_preview(
            root=root_path,
            preview_utc=observed,
            write_outputs=write_outputs,
            refresh_readiness=True,
        )
    design_df = read_o_contract_df(root_path, "restock_po_line_design_preview_lines_live")
    lines_df = _build_lines(observed, design_df)
    summary_df = _build_summary(observed, lines_df)
    source_paths = [
        root_path / "out" / "systems" / "O" / "live" / "restock_po_line_design_preview_lines_live.csv",
    ]
    health_df = _build_health(observed, lines_df, summary_df, source_paths)
    if write_outputs:
        lines_df = write_o_contract_df(root_path, "restock_po_draft_packet_review_lines_live", lines_df)
        summary_df = write_o_contract_df(root_path, "restock_po_draft_packet_review_summary_live", summary_df)
        health_df = write_o_contract_df(root_path, "restock_po_draft_packet_review_health", health_df)
        history_dir = paths.history_dir / f"po_draft_packet_review_v1_{observed.replace(':', '').replace('-', '')}"
        history_dir.mkdir(parents=True, exist_ok=True)
        lines_df.to_csv(history_dir / "restock_po_draft_packet_review_lines_live.csv", index=False)
        summary_df.to_csv(history_dir / "restock_po_draft_packet_review_summary_live.csv", index=False)
        health_df.to_csv(history_dir / "restock_po_draft_packet_review_health.csv", index=False)
    return lines_df, summary_df, health_df


def main() -> int:
    lines_df, summary_df, health_df = build_po_draft_packet_review()
    bad_health = health_df[health_df.get("status", "").map(_normalize_text).ne("ok")]
    print(f"po_draft_packet_review_line_rows={len(lines_df.index)}")
    print(f"po_draft_packet_review_summary_rows={len(summary_df.index)}")
