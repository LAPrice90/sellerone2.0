...
CONTROL_EVENT_CONTRACT = "restock_po_draft_review_control_events"
CONTROL_STATE_CONTRACT = "restock_po_draft_review_controls_live"
CONTROL_HEALTH_CONTRACT = "restock_po_draft_review_controls_health"
DECISION_STATUS = "local_po_draft_review_control"
FILE_SHAPE_READY_STATE = "ready_for_local_po_draft_file_shape_review_only"
MORE_PROOF_STATE = "local_po_draft_more_proof_needed"
HOLD_STATE = "local_po_draft_keep_on_hold"
SHAPE_READY_STATE = "local_po_draft_shape_ready_not_po"
ALLOWED_DECISION_STATES = {MORE_PROOF_STATE, HOLD_STATE, SHAPE_READY_STATE}
NO_DECISION_STATE = "waiting_for_local_po_draft_review_control"
BLOCKED_SOURCE_STATE = "blocked_file_shape_not_ready"
STALE_DECISION_STATE = "blocked_local_po_draft_control_stale"
FALSE_READY_STATE = "blocked_false_local_po_draft_shape_ready"
ZERO_FLAG_COLUMNS = (
...
def _normalize_decision_state(value: object) -> str:
    ...
    aliases = {
        ...
        "local_more_proof": MORE_PROOF_STATE,
        "hold": HOLD_STATE,
        "keep_hold": HOLD_STATE,
        "keep_on_hold": HOLD_STATE,
        "local_hold": HOLD_STATE,
        "shape_ready": SHAPE_READY_STATE,
        "file_shape_ready": SHAPE_READY_STATE,
        "ready": SHAPE_READY_STATE,
        "ready_only": SHAPE_READY_STATE,
    }
...
def _summary_is_shape_ready(row: dict[str, object]) -> bool:
    ...
    return (
        _normalize_text(row.get("file_shape_state", "")) == FILE_SHAPE_READY_STATE
        and line_count != ""
...
def normalize_po_draft_review_control_event(row: dict[str, object]) -> dict[str, str]:
    normalized = {column: _normalize_text(row.get(column, "")) for column in o_contract_columns(CONTROL_EVENT_CONTRACT)}
    normalized["event_utc"] = _normalize_utc(normalized["event_utc"]) or _utc_now_iso()
    normalized["control_event_id"] = normalized["control_event_id"] or f"o-po-draft-control-{uuid.uuid4().hex[:12]}"
    normalized["source_shape_utc"] = _normalize_utc(normalized["source_shape_utc"])
    ...
    normalized["expected_file_shape_value_gbp"] = _money_text(normalized["expected_file_shape_value_gbp"])
    normalized["actor"] = normalized["actor"] or "operator_ui"
    normalized["event_source_reference"] = normalized["event_source_reference"] or "o_ui_po_draft_review_controls"
...
def validate_po_draft_review_control_event(row: dict[str, object]) -> list[str]:
    normalized = normalize_po_draft_review_control_event(row)
    errors: list[str] = []
    ...
    raw_expected_value = _normalize_text(row.get("expected_file_shape_value_gbp", ""))
    if normalized["po_draft_file_shape_preview_id"] == "":
        errors.append("missing_file_shape_preview_id")
    ...
    if raw_expected_value and normalized["expected_file_shape_value_gbp"] == "":
        errors.append("invalid_expected_file_shape_value_gbp")
    if raw_decision_status != DECISION_STATUS:
        errors.append("decision_status_must_be_local_po_draft_review_control")
    for column in ZERO_FLAG_COLUMNS:
...
def build_po_draft_review_control_event_row(
    *,
    file_shape_summary_row: dict[str, object],
    decision_state: object,
    decision_note: object = "",
    actor: str = "operator_ui",
    event_source_reference: str = "o_ui_po_draft_review_controls",
) -> dict[str, str]:
    source = {key: _normalize_text(value) for key, value in dict(file_shape_summary_row).items()}
    row = {
        "event_utc": _utc_now_iso(),
        "control_event_id": f"o-po-draft-control-{uuid.uuid4().hex[:12]}",
        ...
        "source_shape_utc": source.get("shape_utc", ""),
        ...
        "expected_file_shape_value_gbp": source.get("file_shape_value_gbp", ""),
    ...
    for column in ZERO_FLAG_COLUMNS:
        row[column] = "0"
    normalized = normalize_po_draft_review_control_event(row)
    errors = validate_po_draft_review_control_event(normalized)
    if errors:
        raise ValueError(";".join(errors))
    return normalized


def submit_po_draft_review_control_event(
    *,
    root: Path | None = None,
    file_shape_summary_row: dict[str, object],
    decision_state: object,
    ...
    if not _all_zero_flags(dict(file_shape_summary_row)):
        raise ValueError("local_po_draft_review_control_requires_zero_action_flags")
    if normalized_state == SHAPE_READY_STATE and not _summary_is_shape_ready(dict(file_shape_summary_row)):
        raise ValueError("local_po_draft_shape_ready_requires_ready_file_shape")
    root_path = Path(root) if root is not None else get_o_path_contract().root
    ensure_o_directories(root=root_path)
    row = build_po_draft_review_control_event_row(
        file_shape_summary_row=file_shape_summary_row,
        decision_state=decision_state,
        decision_note=decision_note,
        actor=actor,
        event_source_reference=event_source_reference,
    )
    return append_o_contract_row(root_path, CONTROL_EVENT_CONTRACT, row)


def latest_po_draft_review_control_events(events_df: pd.DataFrame) -> pd.DataFrame:
    if events_df.empty:
        return pd.DataFrame(columns=o_contract_columns(CONTROL_EVENT_CONTRACT))
    work = events_df.copy()
    for column in o_contract_columns(CONTROL_EVENT_CONTRACT):
        if column not in work.columns:
            work[column] = ""
        work[column] = work[column].map(_normalize_text)
    good_mask = []
    for _, row in work.iterrows():
        good_mask.append(validate_po_draft_review_control_event(row.to_dict()) == [])
    ...
    work = work.sort_values(by=["_event_sort", "control_event_id"], ascending=[False, False], kind="stable")
    work = work.drop_duplicates(subset=["po_draft_file_shape_preview_id"], keep="first")
...
def _build_controls(control_utc: str, summary_df: pd.DataFrame, latest_events_df: pd.DataFrame) -> pd.DataFrame:
    if summary_df.empty:
        return pd.DataFrame()
    latest_by_shape = {
        _normalize_text(row.get("po_draft_file_shape_preview_id", "")): row.to_dict()
        for _, row in latest_events_df.iterrows()
        if _normalize_text(row.get("po_draft_file_shape_preview_id", ""))
    }
    rows: list[dict[str, str]] = []
    for _, summary_row in summary_df.iterrows():
        summary = {key: _normalize_text(value) for key, value in summary_row.to_dict().items()}
        preview_id = summary.get("po_draft_file_shape_preview_id", "")
        latest = latest_by_shape.get(preview_id, {})
        latest_state = _normalize_text(latest.get("decision_state", "")) or NO_DECISION_STATE
        reasons: list[str] = []
        shape_ready = _summary_is_shape_ready(summary)
        if not _all_zero_flags(summary):
            control_state = BLOCKED_SOURCE_STATE
            reasons.append("source_action_flags_not_zero")
        elif not shape_ready and latest_state == SHAPE_READY_STATE:
            control_state = FALSE_READY_STATE
            reasons.append("shape_ready_decision_without_ready_file_shape")
        elif not shape_ready:
            control_state = BLOCKED_SOURCE_STATE
            for reason in summary.get("file_shape_block_reasons", "").split("|"):
                if reason:
                    reasons.append(reason)
            if not reasons:
                reasons.append("file_shape_not_ready")
        elif latest_state == NO_DECISION_STATE:
            control_state = NO_DECISION_STATE
            reasons.append("no_local_po_draft_review_control")
        elif latest_state == SHAPE_READY_STATE:
            if _event_matches_summary(latest, summary):
                control_state = SHAPE_READY_STATE
            else:
                control_state = STALE_DECISION_STATE
                reasons.append("review_control_file_shape_stale")
        elif latest_state == HOLD_STATE:
            control_state = HOLD_STATE
            reasons.append("kept_on_local_hold")
        elif latest_state == MORE_PROOF_STATE:
            control_state = MORE_PROOF_STATE
            reasons.append("more_proof_needed")
        else:
            control_state = NO_DECISION_STATE
            reasons.append("unsupported_latest_decision_state")
        row = {
            "control_utc": control_utc,
            ...
            "source_shape_utc": summary.get("shape_utc", ""),
            ...
            "file_shape_value_gbp": summary.get("file_shape_value_gbp", ""),
            "source_file_shape_state": summary.get("file_shape_state", ""),
            "latest_decision_state": latest_state,
            "latest_control_event_id": _normalize_text(latest.get("control_event_id", "")),
            "latest_decision_utc": _normalize_text(latest.get("event_utc", "")),
            "review_control_state": control_state,
            "review_control_reasons": "|".join(dict.fromkeys(reasons)),
...
def _build_health(
    ...
    for _, event in events_df.iterrows():
        event_id = _normalize_text(event.get("control_event_id", "")) or "missing_event"
        errors = validate_po_draft_review_control_event(event.to_dict())
        if errors:
    ...
    for _, row in controls_df.iterrows():
        preview_id = _normalize_text(row.get("po_draft_file_shape_preview_id", "")) or "missing_shape"
        if any(_normalize_text(row.get(column, "")) != "0" for column in ZERO_FLAG_COLUMNS):
            live_action_rows.append(preview_id)
        if _contains_unsafe_language(
            row.get("review_control_state", ""),
            row.get("review_control_reasons", ""),
            row.get("latest_decision_note", ""),
        ):
            live_language_rows.append(preview_id)
        control_state = _normalize_text(row.get("review_control_state", ""))
        if control_state == FALSE_READY_STATE:
            false_ready_rows.append(preview_id)
        if control_state == SHAPE_READY_STATE and (
            _normalize_text(row.get("source_file_shape_state", "")) != FILE_SHAPE_READY_STATE
            ...
            false_ready_rows.append(preview_id)
        if control_state != SHAPE_READY_STATE and _normalize_text(row.get("review_control_reasons", "")) == "":
    ...
    checks = [
        (
            ...
            "Every PO draft review-control event must be local-only and complete.",
        ),
        (
            "local_only_guard",
            ...
            "PO draft review controls must not write PO files, write PO hold files, create POs, commit buying, receive stock, or send to Amazon.",
        ),
        (
            "control_claim_guard",
            ...
            "A packet can only be shape-ready after safe file-shape proof; non-ready states must explain why.",
        ),
        (
            "summary_matches_controls",
            len(controls_df.index) <= len(summary_df.index),
            f"control_rows={len(controls_df.index)};summary_rows={len(summary_df.index)}",
            "PO draft review controls must not invent packets beyond file-shape summaries.",
        ),
    ]
...
def build_po_draft_review_controls(
    root: Path | None = None,
    *,
    control_utc: str | None = None,
    write_outputs: bool = True,
    refresh_construction_summary: bool = True,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    root_path = Path(root) if root is not None else get_o_path_contract().root
    paths = ensure_o_directories(root=root_path)
    observed = control_utc or _utc_now_iso()
    if refresh_construction_summary:
        build_po_preview_construction_summary(
            root=root_path,
            summary_utc=observed,
            write_outputs=write_outputs,
            refresh_file_shape=True,
        )
    summary_df = read_o_contract_df(root_path, "restock_po_draft_file_shape_preview_summary_live")
    events_df = read_o_contract_df(root_path, CONTROL_EVENT_CONTRACT)
    event_path = root_path / "out" / "systems" / "O" / "live" / "restock_po_draft_review_control_events.csv"
    if write_outputs and not event_path.exists():
        events_df = write_o_contract_df(root_path, CONTROL_EVENT_CONTRACT, events_df)
    latest_events_df = latest_po_draft_review_control_events(events_df)
    controls_df = _build_controls(observed, summary_df, latest_events_df)
    source_paths = [
        root_path / "out" / "systems" / "O" / "live" / "restock_po_draft_file_shape_preview_summary_live.csv",
        event_path,
    ]
    health_df = _build_health(observed, controls_df, events_df, summary_df, source_paths)
    if write_outputs:
        controls_df = write_o_contract_df(root_path, CONTROL_STATE_CONTRACT, controls_df)
        health_df = write_o_contract_df(root_path, CONTROL_HEALTH_CONTRACT, health_df)
        history_dir = paths.history_dir / f"po_draft_review_controls_v1_{observed.replace(':', '').replace('-', '')}"
        history_dir.mkdir(parents=True, exist_ok=True)
        controls_df.to_csv(history_dir / "restock_po_draft_review_controls_live.csv", index=False)
        health_df.to_csv(history_dir / "restock_po_draft_review_controls_health.csv", index=False)
    return controls_df, health_df


def main() -> int:
    controls_df, health_df = build_po_draft_review_controls()
    event_rows = read_o_contract_df(get_o_path_contract().root, CONTROL_EVENT_CONTRACT)
    bad_health = health_df[health_df.get("status", "").map(_normalize_text).ne("ok")]
    print(f"po_draft_review_control_rows={len(controls_df.index)}")
    print(f"po_draft_review_control_event_rows={len(event_rows.index)}")
