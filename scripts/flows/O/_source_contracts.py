...
_SOURCE_CONTRACTS: Dict[str, SourceContract] = {
    ...
    "inbound_costs_allocated_sku": SourceContract(
        source_path="out/inbound_costs_allocated_sku.csv",
        owner_flow="B/D finance bridge",
        phase1_requirement="optional",
        required_columns=(
            "shipment_id",
            "sku",
        ...
        fallback_rules=(
            "If unavailable or no SKU allocation exists, O must label inbound/FBA send-cost confidence as missing.",
            "Missing inbound cost confidence must block clean buy wording; it must not be treated as zero cost.",
        ),
        must_not_duplicate=(
            "inbound_cost_allocation_truth",
            "fba_send_cost_truth",
        ),
    ),
