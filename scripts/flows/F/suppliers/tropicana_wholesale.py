    ...
    for candidate in candidates:
        ...
        for header in headers:
            if key and key in _normalize_key(header):
                return header
    raise ValueError(f"missing expected Tropicana column. Tried: {', '.join(candidates)}")
...
def convert_supplier(
    ...
    *,
    ...
    brand_col = _find_optional_column(headers, ["Brand name", "Brand"])
    sku_col = _find_column(headers, ["Sku code", "SKU", "Product Code", "Item Code"])
    title_col = _find_column(headers, ["Name", "Description", "Product Name", "Title"])
    stock_col = _find_optional_column(headers, ["Actual quantity", "Stock", "Stock Level", "Available"])
    category_col = _find_optional_column(headers, ["Product group description", "Category", "Product Group"])
    barcode_col = _find_column(headers, ["Barcode", "EAN", "UPC"])
    cost_col = _find_optional_column(
        headers,
        ["Cost", "Unit Cost", "Unit Price", "Price", "Trade Price", "Base Price", "Wholesale Price"],
    )
    ...
    for _, row in df.iterrows():
        ...
        brand = _normalize_text(row.get(brand_col, "")) if brand_col else ""
        barcode = _clean_barcode(row.get(barcode_col, ""))
        stock = _clean_stock(row.get(stock_col, "")) if stock_col else ""
        category = _normalize_text(row.get(category_col, "")) if category_col else ""
        cost = _clean_price(row.get(cost_col, "")) if cost_col else ""
        ...
        base = {
            ...
            "category": category,
            "notes": "source_type=tropicana_stock_export",
        }
        ...
        base["row_hash"] = _row_hash([supplier_id, sku, barcode, cost, title, brand])
