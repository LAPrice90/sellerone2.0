...
DURABILITY_LEDGER_FIELDS = [
    ...
    "items_rows_in_page",
    "unresolved_orders",
    "retry_queue_persisted",
    "items_checkpoint_written",
    "orders_checkpoint_written",
    "marker_advanced",
    "notes",
]
...
def _append_durability_ledger(row: Dict[str, object]) -> None:
    payload = {k: str(row.get(k, "")).strip() for k in DURABILITY_LEDGER_FIELDS}
    payload["timestamp_utc"] = payload.get("timestamp_utc", "") or datetime.now(timezone.utc).isoformat()
    DURABILITY_LEDGER_PATH.parent.mkdir(parents=True, exist_ok=True)
    need_header = not DURABILITY_LEDGER_PATH.exists() or DURABILITY_LEDGER_PATH.stat().st_size == 0
    with DURABILITY_LEDGER_PATH.open("a", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=DURABILITY_LEDGER_FIELDS)
        if need_header:
            writer.writeheader()
        writer.writerow(payload)


def _persist_retry_queue_durable(
    df: pd.DataFrame,
    *,
    run_id: str,
    page: int | None,
    ...
    _save_retry_queue(df)
    _append_durability_ledger(
        {
            "run_id": run_id,
            "stage": "retry_persist",
            "page": "" if page is None else str(page),
            "retry_queue_persisted": "1",
            "notes": notes,
        }
    )
...
def main() -> None:
    ...
    ingest_run_id = f"B001-{started_at.strftime('%Y%m%dT%H%M%SZ')}"
    ...
    try:
        ...
        while True:
            ...
            for idx, o in enumerate(batch, 1):
                ...
                while attempt < QUOTA_MAX_RETRIES:
                    ...
                    try:
                        ...
                        if attempt < QUOTA_MAX_RETRIES and ("QuotaExceeded" in msg or "429" in msg):
                            ...
                            time.sleep(backoff)
                            continue
                        if order_id and order_id not in failed_orders:
                            failed_orders.append(order_id)
                        retry_df = _enqueue_retry(retry_df, order_id, msg)
                        _persist_retry_queue_durable(
                            retry_df,
                            run_id=ingest_run_id,
                            page=page,
                            notes=f"enqueue_retry_exception order_id={order_id}",
                        )
                        last_error = str(exc)
                ...
                if not success:
                    ...
                    _persist_retry_queue_durable(
                        ...
                        run_id=ingest_run_id,
                        page=page,
                        notes=f"enqueue_retry_no_items order_id={order_id}",
                    )
                else:
                    if order_id:
            ...
            # Enforce a durable completeness contract before admitting headers for this page.
            page_order_ids = sorted(
                {
                    str(r.get("AmazonOrderId", "")).strip()
                    for r in batch
                    if str(r.get("AmazonOrderId", "")).strip()
                }
            )
            unresolved_orders = sorted([oid for oid in page_order_ids if oid not in existing_item_order_ids])
            retry_persisted = False
            items_checkpoint_written = False
            orders_checkpoint_written = False
            marker_advanced = False
            if unresolved_orders:
                retry_ids = set(retry_df.get("order_id", pd.Series([], dtype=str)).astype(str).str.strip())
                missing_retry_ids = [oid for oid in unresolved_orders if oid not in retry_ids]
                for oid in missing_retry_ids:
                    retry_df = _enqueue_retry(retry_df, oid, "page_unresolved_item_obligation")
                _persist_retry_queue_durable(
                    retry_df,
                    run_id=ingest_run_id,
                    page=page,
                    notes=(
                        f"page_unresolved_obligation unresolved={len(unresolved_orders)} "
                        f"added_to_retry={len(missing_retry_ids)}"
                    ),
                )
                retry_persisted = True

            # Incremental checkpoint: write item rows first, then headers.
            try:
                ...
                orders_checkpoint_written = True
            except Exception as exc_ckpt:
                log(f"checkpoint write failed: {exc_ckpt}")

            # Advance marker only after successful page checkpoint.
            if (not SKIP_MARKER_WRITE) and items_checkpoint_written and orders_checkpoint_written:
                try:
                    ...
                    pass
            elif (not SKIP_MARKER_WRITE) and not (items_checkpoint_written and orders_checkpoint_written):
                log("marker advance skipped due to incomplete checkpoint write")

            _append_durability_ledger(
                {
                    "run_id": ingest_run_id,
                    "stage": "page_checkpoint",
                    "page": str(page),
                    "orders_in_page": str(len(df_page_orders.index)),
                    "items_rows_in_page": str(len(df_page_items.index)),
                    "unresolved_orders": str(len(unresolved_orders)),
                    "retry_queue_persisted": "1" if retry_persisted else "0",
                    "items_checkpoint_written": "1" if items_checkpoint_written else "0",
                    "orders_checkpoint_written": "1" if orders_checkpoint_written else "0",
                    "marker_advanced": "1" if marker_advanced else "0",
                    "notes": f"fetched_item_orders={fetched_item_orders};skipped_item_fetch={skipped_item_fetch}",
                }
            )
