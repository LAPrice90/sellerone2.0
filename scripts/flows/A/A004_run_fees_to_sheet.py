...
def main() -> None:
    ...
    stock_available_idx = idx.get("stock_available", -1)
    stock_total_idx = idx.get("stock_total", -1)
    ...
    def _stock_positive(row: list[str]) -> bool:
        for col_idx in (stock_available_idx, stock_total_idx):
    ...
    print(f"[A004] Processing up to {('all' if LIMIT == 0 else LIMIT)} eligible SKUs (rows: {total_skus})")
