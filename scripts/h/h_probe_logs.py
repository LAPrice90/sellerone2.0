...
PROBE_EVENT_LOG_PATH = resolve_compat_path("h_worker_probe_event_log.csv").live_path
PROBE_RESPONSE_LOG_PATH = resolve_compat_path("h_worker_probe_response_log.csv").live_path
...
def _atomic_write_csv(df: pd.DataFrame, target: Path) -> None:
    ...
    mapped = resolve_compat_path(target.name, default_system="H")
    target_norm = str(target).replace("\\", "/")
    live_norm = str(mapped.live_path).replace("\\", "/")
    legacy_norm = str(mapped.legacy_path).replace("\\", "/")
    if target_norm == live_norm and legacy_norm != live_norm:
        mapped.legacy_path.parent.mkdir(parents=True, exist_ok=True)
        df.to_csv(mapped.legacy_path, index=False)
    elif target_norm == legacy_norm and live_norm != legacy_norm:
        mapped.live_path.parent.mkdir(parents=True, exist_ok=True)
        df.to_csv(mapped.live_path, index=False)
