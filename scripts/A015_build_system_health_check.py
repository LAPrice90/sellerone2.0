...
H_PRICING_STATE_PATH = OUT / "h_pricing_cycle_state.json"
...
def _b_cycle_recent_fail_stats(path: Path, window_hours: float) -> Dict[str, object]:
    ...
    events = _iter_b_cycle_log_events(path, cutoff=cutoff)
    fail_events = [event for event in events if str(event.get("event", "")) == "fail"]
...
def _h_floor_policy_checks(rows: List[Dict[str, str]], now_utc: datetime) -> None:
    ...
    missing_ref_rows = tr.loc[missing_referral].copy()
    missing_ref_count = len(missing_ref_rows.index)
    coverage_status = "ok"
    if missing_ref_count > 0:
        coverage_status = "warn" if missing_ref_count < len(tr.index) else "fail"
    _add(
        rows,
        ...
        (
            f"date={today};rows_today={len(tr.index)};sample="
            f"{','.join(missing_ref_rows.get('sku', pd.Series([], dtype=str)).astype(str).head(5).tolist())}"
        ),
    )
...
def _phase1_rollout_checks(rows: List[Dict[str, str]], now_utc: datetime) -> None:
    ...
    if scope.empty:
        ...
        state_payload = _read_json(H_PRICING_STATE_PATH, default={})
        if not state_payload:
            _add(rows, "h_scope_non_parked_matches_targets", "warn", "missing_state", f"path {H_PRICING_STATE_PATH}")
        else:
...
def main() -> None:
    ...
    # Recent log health
    log_path = OUT / "B_cycle.log"
    log_info = _file_info(log_path)
    _add(rows, "b_cycle_log_exists", "ok" if log_info["exists"] == "yes" else "fail", log_info["exists"], f"mtime {log_info['mtime_utc']}")
    if log_path.exists():
        ...
        _add(
            ...
            (
                f"window_hours {window_hours}; raw_fail_count={raw_fail_count}; "
                f"recovered_count={recovered_count}; unresolved_count={unresolved_count}{sample_text}"
            ),
        )
    ...
    missing_input_asof = [name for name, val in input_asof.items() if not val]
    if missing_input_asof:
        _add(rows, "h_e_inputs_fresh", "warn", "missing_asof", ",".join(missing_input_asof))
        expected_input_asof = ""
    else:
        unique_input_dates = sorted(set(input_asof.values()))
        ...
        if len(unique_input_dates) > 1:
            _add(
                ...
                "; ".join([f"{k}={v}" for k, v in input_asof.items()]),
            )
        else:
            try:
                ...
                _add(
                    ...
                    freshness_status,
                    str(age_days),
                    f"asof_date={expected_input_asof}; max_age_days={max_age_days}",
                )
            except Exception:
                _add(rows, "h_e_inputs_fresh", "warn", "bad_date", f"asof_date={expected_input_asof}")

    if not expected_input_asof:
        ...
        if bad_outputs:
            _add(
                ...
                "warn",
                str(len(bad_outputs)),
                f"expected={expected_input_asof}; mismatch={','.join(bad_outputs)}; " + "; ".join(notes),
            )
        else:
            _add(rows, "h_e_outputs_latest_asof", "ok", "0", f"expected={expected_input_asof}")
    ...
    df_profile = df_all.loc[_profile_filter_mask(df_all, profile)].copy()
    ...
    print({"status": "success", "rows": len(df_profile), "snapshot": str(checklist_path), "profile": profile})
