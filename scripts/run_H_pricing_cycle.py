...
from h_probe_logs import append_probe_events, append_probe_responses, initialize_probe_logs, load_probe_event_log
from h_supervisor_tactical_rules import load_active_supervisor_tactical_rules
try:
    from scripts.h_floor_policy import cogs_cost_from_exvat, gross_from_exvat, load_h_floor_vat_policy
except ModuleNotFoundError:
    from h_floor_policy import cogs_cost_from_exvat, gross_from_exvat, load_h_floor_vat_policy
try:
...
LOCK_PATH = OUT / "H_pricing_cycle.lock"
LOG_PATH = OUT / "H_pricing_cycle.log"
H_CYCLE_LOG_PATH = OUT / "H_cycle.log"
STATE_PATH = OUT / "h_pricing_cycle_state.json"
...
SNAPSHOT_REFRESH_SCRIPT = ROOT / "scripts" / "H001_capture_offer_snapshot.py"
...
PHASE1_PILOT_TIMEOUT_SECONDS = max(float(os.environ.get("H_PHASE1_PILOT_TIMEOUT_SECONDS", "1800") or "1800"), 30.0)
...
def _log(message: str) -> None:
    ...
    line = f"{_ts()} {message}"
    for path in (LOG_PATH, H_CYCLE_LOG_PATH):
        try:
...
def _acquire_lock() -> None:
    ...
    if LOCK_PATH.exists() and not force:
        payload = _norm(LOCK_PATH.read_text(encoding="utf-8"))
...
def _write_lock() -> None:
    ...
    now = _ts()
    LOCK_PATH.write_text(f"H|pid={os.getpid()}|start={now}|heartbeat={now}\n", encoding="utf-8")
...
def _ensure_lock_ownership() -> None:
    ...
    _write_lock()
    _log(f"stale lock recovered (prior_pid={pid if pid is not None else ''})")
...
def _active_sku_asin_rows() -> List[Dict[str, str]]:
    active: List[str] = []
...
def _refresh_offer_snapshots(now_utc: datetime, state: dict) -> dict:
    ...
    sku_asin_rows = _active_sku_asin_rows()
...
def _run_phase1_pilot_subprocess(*, now_utc: datetime, run_id: str, config_path: str, read_only: bool) -> dict[str, str]:
    cmd = [
        sys.executable,
        str(ROOT / "scripts" / "H110_run_phase1_h_pilot.py"),
        "--phase1-config",
        str(config_path),
        "--run-id",
        run_id,
    ...
    try:
        proc = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=PHASE1_PILOT_TIMEOUT_SECONDS,
        )
    except subprocess.TimeoutExpired as exc:
        raise RuntimeError(f"phase1 pilot step timeout after {PHASE1_PILOT_TIMEOUT_SECONDS:.0f}s") from exc
...
def _run_h_profile_health_check() -> dict[str, str]:
    ...
    if proc.returncode != 0 and rc == 2:
        stderr = _norm(proc.stderr or "")
        stdout = _norm(proc.stdout or "")
        out["error"] = (stderr or stdout or f"rc={proc.returncode}")[:300]
    return out
...
def _run_phase1_daily_intel_alignment_subprocess(*, now_utc: datetime, run_id: str, config_path: str) -> dict[str, str]:
    cmd = [
        sys.executable,
        str(ROOT / "scripts" / "A016_refresh_phase1_daily_intel.py"),
        "--phase1-config",
        str(config_path),
        "--scope",
        "full_db",
    ...
    try:
        proc = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=PHASE1_ALIGNMENT_TIMEOUT_SECONDS,
        )
            ...
            try:
                ...
                if args.phase1_pilot:
                    refresh_state = _run_with_retries(
                        ...
                        lambda: _refresh_offer_snapshots(now_utc, state),
                    )
                    if refresh_state:
                ...
                time.sleep(H_LOOP_ERROR_SLEEP_SECONDS)
    finally:
        _release_lock()
    return 0
