...
def sync_level1_cogs(token_df: pd.DataFrame, client: gspread.Client) -> None:
...
def main() -> None:
        ...
        if shortage_by_sku:
            ...
            if adjusted:
                ...
                pd.DataFrame(out_rows).to_csv(
                    "out/token_shortages_by_sku.csv", index=False
                )
            else:
                pd.DataFrame(
                    columns=["timestamp", "seller_sku", "missing_qty"]
                ).to_csv("out/token_shortages_by_sku.csv", index=False)
        else:
            # Write empty file to avoid stale shortage alerts.
            pd.DataFrame(columns=["timestamp", "seller_sku", "missing_qty"]).to_csv(
                "out/token_shortages_by_sku.csv", index=False
            )
    ...
    if not new_allocations:
        ...
        alloc_out.to_csv("out/token_allocations_live.csv", index=False)
    ...
    combined_alloc.to_csv("out/token_allocations_live.csv", index=False)
    updated_tokens.drop(columns=["received_date_dt", "token_seq"], errors="ignore").to_csv(
        "out/token_ledger_live.csv", index=False
    )
    ...
    if log_rows and not SKIP_TOKEN_SHEETS:
        try:
