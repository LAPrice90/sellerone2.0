...
class OasDecisionResult:
    ...
    admissible_flag: str
    suppression_threshold_update_allowed_flag: str
...
def evaluate_oas_hard_fails(
    *,
    ...
    suppression_active_flag: object = "0",
) -> OasDecisionResult:
    ...
    suppression_active = _truthy(suppression_active_flag)
    if suppression_active:
        reason_codes.append("OAS_BLOCK_GENERAL_LEARNING_SUPPRESSION_WINDOW")
    ...
    threshold_update_blockers = [
        "OAS_FAIL_MARKET_STRUCTURE_CHANGED",
        "OAS_FAIL_WRITER_CONFLICT",
        "OAS_FAIL_PROMO_SUSPECTED",
        "OAS_FAIL_FEATURED_WINNER_DELIVERY_UNKNOWN",
    ]
    threshold_allowed = "1" if suppression_active and not any(code in reason_codes for code in threshold_update_blockers) else "0"
    return OasDecisionResult(
        context_quality_score=score,
        admissible_flag=admissible,
        suppression_threshold_update_allowed_flag=threshold_allowed,
