...
def _load_scope_parking_sets(scope_path: Path = DEFAULT_SCOPE_PATH) -> tuple[set[str] | None, set[str], set[str]]:
    if not scope_path.exists():
        return None, set(), set()
    try:
        scope = pd.read_csv(scope_path, dtype=str).fillna("")
    ...
    if "sku" not in scope.columns or "parked_flag" not in scope.columns:
        return None, set(), set()
    scope["sku_key"] = scope["sku"].astype(str).str.strip().str.upper()
    scope["parked_key"] = scope["parked_flag"].astype(str).str.strip()
    all_scope_skus = {s for s in scope["sku_key"].tolist() if s}
    parked_scope_skus = {s for s in scope.loc[scope["parked_key"].eq("1"), "sku_key"].tolist() if s}
    non_parked_scope_skus = {s for s in scope.loc[~scope["parked_key"].eq("1"), "sku_key"].tolist() if s}
    return non_parked_scope_skus, parked_scope_skus, all_scope_skus
...
def resolve_target_universe(
    ...
    *,
    ...
    if mode == "single_sku":
        ...
        non_parked_scope_skus, parked_scope_skus, all_scope_skus = _load_scope_parking_sets(scope_path=scope_path)
        # Scope filter should restrict to in-scope SKUs, but must not drop parked SKUs.
        # Parked SKUs still need market data collection for sell-off observability.
        scope_filter_available = non_parked_scope_skus is not None
        ...
        for sku in candidate_skus:
            ...
            if not _is_in_stock_listing_row(row):
                ...
                continue
            if scope_filter_available and sku not in all_scope_skus:
                if sku not in parked_scope_skus:
                    skipped_non_scope_count += 1
                continue
            skus.append(sku)
        if not candidate_skus:
            notes.append("merchant_active_empty_or_missing")
        if scope_filter_available:
            notes.append("active_merchant_scope_filter_applied")
            if skipped_non_scope_count > 0:
                notes.append(f"active_merchant_scope_excluded_non_scope_{skipped_non_scope_count}")
        else:
            notes.append("active_merchant_scope_filter_unavailable")
