...
if not exist "%SCRIPT%" (
  echo [home_time_offline] missing monitor script: "%SCRIPT%"
  endlocal & exit /b 1
)
...
if not defined HOME_TIME_MONITOR_ONCE set "HOME_TIME_MONITOR_ONCE=1"

set "ARGS="
if /I "%HOME_TIME_MONITOR_ONCE%"=="1" set "ARGS=%ARGS% --once"
...
"%PY%" -u "%SCRIPT%" %ARGS%
set "RC=%ERRORLEVEL%"
echo [home_time_offline] monitor exit rc=%RC%
