    ...
    st.markdown(
        _operator_task_brief_html(
            ...
            steps=[
                ...
                ("Send local choices", "Only selected choices are recorded locally after Luke presses Send Choices."),
            ],
...
def _restock_supplier_action_buckets_html(filtered_df: pd.DataFrame) -> str:
    ...
    return (
        ...
        "<div class='o-restock-action-buckets-title'>Supplier action buckets</div>"
        "<div class='o-restock-stats-grid'>"
...
def _render_restock_workbench_overview(filtered_df: pd.DataFrame, *, supplier_label: object = "") -> None:
    import streamlit as st
...
def _render_restock_workbench_table(filtered_df: pd.DataFrame, *, root_path: Path) -> None:
    ...
    st.caption(
        "These cards are the normal working view. Maintenance tables stay below this working view and out of the main path."
    )
    if filtered_df.empty:
        st.info("No rows match the current Restocking filters.")
        return
...
def _render_restock_session_tab(root_path: Path, datasets: dict[str, pd.DataFrame]) -> None:
    ...
    if page_mode != "Admin Proof":
        _render_restock_workbench_overview(filtered_df, supplier_label=supplier_filter)
