...
OPERATOR_PAGE_DESCRIPTIONS = {
    ...
    "new_product_review": "Supplier intake and new product review.",
...
def _render_feeder_review_card(row_data: dict[str, object], *, pack_type: str) -> dict[str, str]:
    ...
    image_html = (
        _image_frame_html(image_url, size=96)
        if image_url
        else "<div class='o-intake-image-placeholder'>No image</div>"
    )
    if amazon_dp_url:
        amazon_link = (
    ...
    with st.container():
        ...
        st.markdown("<div class='o-intake-action-title'>Luke decision</div>", unsafe_allow_html=True)
        action_cols = st.columns([1.0, 1.15, 2.2, 0.75], gap="medium")
        with action_cols[0]:
...
def _render_new_product_review_tab(root_path: Path) -> None:
    ...
    st.markdown(
        "<div class='o-intake-summary-strip'>"
        "<span class='o-intake-summary-label'>Supplier</span>"
        f"<span class='o-intake-summary-value'>{html.escape(active_supplier)}</span>"
        "<span class='o-intake-summary-divider'>|</span>"
        "<span class='o-intake-summary-label'>Clean pass</span>"
        f"<span class='o-intake-summary-value'>{html.escape(clean_pass_rows)}</span>"
        "<span class='o-intake-summary-divider'>|</span>"
        "<span class='o-intake-summary-label'>Manual review</span>"
        f"<span class='o-intake-summary-value'>{html.escape(manual_review_rows)}</span>"
        "<span class='o-intake-summary-divider'>|</span>"
        "<span class='o-intake-summary-label'>Near miss</span>"
        f"<span class='o-intake-summary-value'>{html.escape(near_miss_lane_rows)}</span>"
        "<span class='o-intake-summary-divider'>|</span>"
        "<span class='o-intake-summary-label'>To review</span>"
        f"<span class='o-intake-summary-value'>{html.escape(reviewable_total_rows)}</span>"
        "</div>",
        unsafe_allow_html=True,
    )
...
def _restock_chip_tone(value: object) -> str:
    text = _normalize_text(value).lower()
    if text in {"", "ok", "ready", "verified", "clean", "pack_or_moq_visible", "supplier_stock_verified_in_stock"}:
        return "good"
    if "missing" in text or "not_verified" in text or "blocked" in text or "unknown" in text:
        return "warn"
    return ""
...
def _render_restock_workbench_table(filtered_df: pd.DataFrame) -> None:
    import streamlit as st
...
def _render_restock_session_tab(root_path: Path, datasets: dict[str, pd.DataFrame]) -> None:
    ...
    if page_mode != "Admin Proof":
        ...
        _render_restock_workbench_table(filtered_df)
