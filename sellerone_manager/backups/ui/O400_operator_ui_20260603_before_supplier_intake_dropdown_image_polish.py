...
def _render_feeder_review_card(row_data: dict[str, object], *, pack_type: str) -> dict[str, str]:
    ...
    with st.container():
        ...
        action_cols = st.columns([1.0, 1.2, 2.35, 0.9], gap="medium")
        with action_cols[0]:
...
def _render_new_product_review_tab(root_path: Path) -> None:
    ...
    action_cols = st.columns([1.25, 1.25, 4.0])
    ...
    with st.expander("Recently sent choices"):
        if sent_df.empty:
            st.caption("No choices have been sent for the current filters.")
        else:
