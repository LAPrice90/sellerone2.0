...
def _render_restock_site_overview(review_df: pd.DataFrame, summary_df: pd.DataFrame) -> None:
    import streamlit as st
    ...
    path_cols = st.columns(3, gap="small")
...
def _render_restock_session_tab(root_path: Path, datasets: dict[str, pd.DataFrame]) -> None:
    ...
    if page_mode == "Overview":
        _render_restock_site_overview(review_df, summary_df)
        return
