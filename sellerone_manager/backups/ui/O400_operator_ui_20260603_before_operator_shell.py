...
def _normalize_feeder_review_decision(value: object) -> str:
    ...
    if token in {"pass", "fail"}:
        return token
    if token in {"rescan", "re scan", "retry", "scan again", "recheck", "re check"}:
        return "rescan"
    return ""
...
def _feeder_review_pack_work_phrase(lane_label: str, count: int, *, unique: bool = False) -> str:
    ...
    if lane == "passes":
        ...
        return f"{count} {uniqueness}{'pass' if count == 1 else 'passes'} to review"
...
def _seed_feeder_review_widget_draft(
    ...
    *,
    ...
    decision_display = FEEDER_REVIEW_DECISION_DISPLAY.get(draft_decision, "Select")
...
def _render_new_product_review_tab(root_path: Path) -> None:
    ...
    if reviewed_rows:
        st.caption(
            "Current window selections: "
            f"pass {pass_selected} | fail {fail_selected} | re scan {rescan_selected} | "
            f"remaining {len(reviewed_rows) - pass_selected - fail_selected - rescan_selected}"
        )
        st.caption(
...
def _render_restock_session_tab(root_path: Path, datasets: dict[str, pd.DataFrame]) -> None:
    ...
    st.subheader("Restock Session")
    st.caption("Local review session only. It gathers proof and labels blockers; it does not create purchase orders.")

    refresh_col, status_col = st.columns([1, 3])
    if refresh_col.button("Refresh Session Proof", key="o_restock_session_refresh"):
        ...
        st.session_state["o_recent_submit_notice"] = "Restock session proof refreshed locally."
        st.rerun()
    ...
    if health_df.empty:
        status_col.info("No restock session proof has been built yet.")
    elif bad_health.empty:
        status_col.success("Session proof is local and safe.")
    else:
        status_col.warning("Session proof has warnings or blockers.")
    ...
    for col in ("supplier_name", "source_class", "row_status", "seller_sku", "asin", "title", "supplier_sku", "barcode"):
        if col not in review_df.columns:
            review_df[col] = ""
    review_df["_supplier_label"] = review_df["supplier_name"].map(_supplier_label)
    supplier_options = ["All suppliers", *sorted(review_df["_supplier_label"].unique().tolist())]
    source_options = ["All sources", *sorted({_normalize_text(value) for value in review_df["source_class"].tolist() if _normalize_text(value)})]
    status_options = ["All rows", *sorted({_normalize_text(value) for value in review_df["row_status"].tolist() if _normalize_text(value)})]

    controls_a, controls_b, controls_c, controls_d = st.columns([1.2, 1.1, 1.1, 2.2])
    supplier_filter = controls_a.selectbox("Supplier", options=supplier_options, index=0, key="o_session_supplier_filter")
    source_filter = controls_b.selectbox("Source", options=source_options, index=0, key="o_session_source_filter")
    row_status_filter = controls_c.selectbox("State", options=status_options, index=0, key="o_session_status_filter")
    search_text = controls_d.text_input("Search SKU / Title / ASIN", value="", key="o_session_search")
    ...
    st.caption(
        f"Showing {len(filtered_df.index)} rows across {filtered_df['_supplier_label'].nunique()} suppliers. "
    ...
    with st.expander("Draft decision", expanded=True):
    ...
    display_columns = [col for col in RESTOCK_SESSION_DISPLAY_COLUMNS if col in filtered_df.columns]
    for supplier_label, supplier_df in filtered_df.groupby("_supplier_label", sort=True):
        st.markdown(f"### {supplier_label}")
        st.dataframe(supplier_df[display_columns], width="stretch", hide_index=True)
