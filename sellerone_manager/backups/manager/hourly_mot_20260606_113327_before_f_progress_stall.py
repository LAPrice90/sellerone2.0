...
def _h_defensive_listing_protection_rows(*, base: Path, observed_utc: str) -> list[dict[str, str]]:
    ...
    if config_rows is None:
        ...
        historical_stand_down_violation_rows = []
        latest_stand_down_violation = False
        for row in b06_proof_rows:
            current = _h_money_float(row.get("current_price_gbp"))
            rival = _h_money_float(row.get("lowest_rival_price_gbp"))
            if (
                row.get("write_status") == "APPLIED"
                and current is not None
                and rival is not None
                and rival >= current
            ):
                if row is latest_b06_proof:
                    latest_stand_down_violation = True
                else:
                    historical_stand_down_violation_rows.append(row)
        ...
        if invalid_rows:
        ...
        elif latest_stand_down_violation:
            status = "fail"
            value = (
                "latest_stand_down_violation=1;"
            ...
            root_cause = "The latest defensive listing proof row applied a write even though the rival was equal to or above us."
        ...
        actual = (
            ...
            f"historical_stand_down_violation_rows={len(historical_stand_down_violation_rows)}"
        )
...
def _f_seller_central_eligibility_auth_rows(*, base: Path, observed_utc: str, now: datetime) -> list[dict[str, str]]:
    ...
    if not proof_path.exists():
        ...
        luke_needed = False
    elif proof_status in {"disabled", "waiting_for_code", "blocked"}:
        status = "decision_needed"
        value = proof_reason or proof_status
        root_cause = "Seller Central eligibility login needs credentials, SMS forwarding, or live proof approval."
        manager_action = "Luke must complete the missing login/code-forwarding step before live proof continues."
        luke_needed = True
...
def _b_stock_receipt_token_sync_rows(*, base: Path, observed_utc: str, now: datetime) -> list[dict[str, str]]:
    ...
    if schema_errors:
        ...
        if receipt_intake_disabled and receipt_intake_stale:
