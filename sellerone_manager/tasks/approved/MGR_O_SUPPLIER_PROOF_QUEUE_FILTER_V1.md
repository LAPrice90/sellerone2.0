# O Supplier Proof Queue Filter v1

## Manager Authority
- task_id: MGR_O_SUPPLIER_PROOF_QUEUE_FILTER_V1
- job_ref: O-SUPPLIER-QUEUE-FILTER
- flow: O
- task_type: bounded_o_construction
- priority: high
- status: proved
- authority: luke_requested_continue_safe_o_tasks_after_supplier_proof_work_queue
- luke_action_required: 0

## Plain-English Purpose
Add a read-only supplier proof queue focus filter.

This lets the Restock Session page narrow to the current supplier, all uncovered supplier-proof rows, the top queue supplier, the top local action bucket, or the top supplier plus its main local action.

## Boundary
- allowed_scope: Restock Session supplier review display filter, O MOT read-only check, focused UI/MOT tests, rendered page proof, browser render proof, and active O plan notes.
...
- proof_required: focused UI and MOT tests must pass, Streamlit render must show the supplier proof queue focus filter with 0 exceptions and no event row writes, browser render should show the filter when available, and O MOT must keep O user-working with 0 unsafe buying actions.
...
- stop_condition: Stop if implementation needs any protected action or if the filter fetches files, clears supplier proof, imports supplier files, changes F/O source status, or creates any live action.
...
- focused UI tests for supplier proof queue filtering.
- focused MOT tests for supplier proof queue filtering and O user-working readiness.
- Streamlit AppTest Restock Session Supplier Review render shows the supplier proof queue focus filter with 0 exceptions and no proof-event, draft-event, approval-event, or PO-control row writes.
...
## Expected End State
Luke can focus the Restock Session page on the top supplier-proof queue work without O fetching files, clearing proof, approving buying, creating purchase orders, receiving, or sending stock to Amazon.
...
- Render proof showed `Supplier proof queue focus` with current, uncovered, top supplier, top action, and top supplier plus action options.
...
- Browser proof showed the queue focus filter; opening the dropdown showed current supplier, all uncovered rows, top queue supplier, top queue action, and top supplier plus action.
- Browser proof had 0 relevant console issues.
- O MOT passed with 0 fails and 1 existing stale-proof warning.
- `o_supplier_proof_queue_filter=ok`: `options=5;uncovered=607;top_supplier=Stax;top_supplier_rows=78;top_action=check_later_or_mark_drop;top_action_rows=504;top_supplier_action=check_later_or_mark_drop;top_supplier_action_rows=76`.
