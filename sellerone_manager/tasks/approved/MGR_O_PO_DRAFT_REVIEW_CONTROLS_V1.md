# O PO Draft Review Controls v1

## Manager Authority
- task_id: MGR_O_PO_DRAFT_REVIEW_CONTROLS_V1
- job_ref: O-PO-DRAFT-REVIEW
- flow: O
...
Add local PO draft review controls after the file-shape preview.

This gives the Restock Session UI a safe local control point where a packet can be marked as needing more proof, staying on local hold, or shape-ready for the next local draft-review step. It does not create purchase orders, write PO files, write PO hold files, commit buying, receive stock, or send anything to Amazon.

## Boundary
- allowed_scope: O PO draft review-control contracts, local event capture, local builder, UI preview/control view, O manager/MOT proof, focused tests, and active O plan notes.
...
- proof_required: Targeted O review-control/session/UI/MOT tests must pass, the full local O preview builder chain must run once, and O MOT must keep O user-working with 0 unsafe buying actions.
- retest_command: python -m sellerone_manager.app --hourly-mot --mot-flow O
- rollback_path: Use git diff for code rollback. Do not delete O outputs. If local review-control output is malformed, rebuild through O486 only after upstream local preview files are safe.
...
- `scripts/flows/O/O486_build_po_draft_review_controls.py`
- `scripts/flows/O/_schemas.py`
- `sellerone_manager/hourly_mot.py`
- `tests/test_o486_po_draft_review_controls.py`
...
## Expected End State
O can record and show local PO draft review-control decisions after file-shape preview. The new controls remain local-only and cannot create or commit a buy.

## Proof Result
- proved_utc: 2026-06-03T10:44:20Z
- Full compile passed for O400, O486, O484, O482, O480, O478, O476, O474, O472, O470, O schemas, and O MOT.
- O-scoped proof tests passed: 132 tests, 109 deselected.
