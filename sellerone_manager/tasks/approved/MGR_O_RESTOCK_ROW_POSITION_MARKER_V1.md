# O Restock Row Position Marker v1

## Manager Authority
- task_id: MGR_O_RESTOCK_ROW_POSITION_MARKER_V1
- job_ref: O-RESTOCK-ROW-POSITION
- flow: O
...
Add a small read-only marker to each Restock Session product card explaining why that card is near the top of the filtered list.

This should make the new priority sorting easier to understand without changing any business state.

## Boundary
- allowed_scope: O Restock Session card display, read-only use of existing row blocker/proof fields, focused UI tests, rendered page proof, O MOT proof, and active O plan notes.
...
- proof_required: Focused Restock Session UI tests must pass, Streamlit Restock Session render must show row position markers with 0 exceptions, browser render proof should show the current Restock Session page when available, and O MOT must keep O user-working with 0 unsafe buying actions.
...
- `python -m pytest tests\test_o_ui_operator_view.py -q -k "restock or supplier_file or card_control or supplier_proof or proof_history or missing_proof or supplier_readiness or proof_worklist or next_proof or selected_row_proof or safe_save or action_bucket or priority_sort or position_marker"`
- Streamlit AppTest Restock Session Supplier Review render shows row position markers with 0 exceptions and no proof-event or draft-event row writes.
...
## Expected End State
The selected supplier view explains why each card is high in the current filtered list, without changing rows, facts, decisions, or purchase state.
...
- Browser render proof showed the Restock Session page with the row-position marker, local action bucket filter, safe local save guidance, and product cards.
...
## Result
The selected supplier view now explains why each visible card is where it is in the sorted local work list. This is display-only O construction work, not a purchase-order or restock commitment.
