# O Restock Approval Preview Visibility Bundle v1

## Manager Authority
- task_id: MGR_O_RESTOCK_APPROVAL_PREVIEW_VISIBILITY_BUNDLE_V1
- job_ref: O-RESTOCK-APPROVAL-PREVIEW-02
- flow: O
...
Surface existing approval-preview packet status in the main Restock Session supplier review page.

This should help Luke see whether current filtered rows already exist in a local approval preview, without saving an approval or creating a purchase order.

## Boundary
- allowed_scope: O Restock Session supplier review display, read-only use of existing approval preview outputs, focused UI tests, rendered page proof, O MOT proof, and active O plan notes.
...
- proof_required: Focused Restock Session UI tests must pass, Streamlit Restock Session render must show approval-preview packet status with 0 exceptions, browser render proof should show the current Restock Session page when available, and O MOT must keep O user-working with 0 unsafe buying actions.
...
## Expected End State
The selected supplier view shows existing local approval-preview packet status for the current supplier/filter, without changing rows, facts, decisions, approvals, or purchase state.
...
- The rendered page showed the approval-preview packet status panel and per-card approval-preview status.
...
- Browser render proof showed the Restock Session page with the approval-preview status panel, approval-preview card status, approval-readiness filter, and safe local save guidance.
...
## Result
The selected supplier view now surfaces existing local approval-preview packet status in the main working path. This is display-only O construction work, not an approval event, purchase order, or restock commitment.
