# O Restock Selected Row Proof Drawer v1

## Manager Authority
- task_id: MGR_O_RESTOCK_SELECTED_ROW_PROOF_DRAWER_V1
- job_ref: O-RESTOCK-SELECTED-ROW
- flow: O
...
Add a read-only selected-row proof checklist drawer to the Restock Session supplier review cards.

This tells Luke which local proof fields should be filled before saving supplier proof, without saving proof or changing business state.

## Boundary
- allowed_scope: O Restock Session selected-row proof checklist display, read-only use of existing row blocker/proof fields, focused UI tests, rendered page proof, O MOT proof, and active O plan notes.
- forbidden_actions: no Google Sheets write; no price change; no queue edit; no Product DB or local DB alignment; no supplier file move/delete/rewrite/import/download; no Gmail fetch or attachment download; no F061 run; no F source-status rewrite; no real purchase order; no purchase order file write; no purchase order hold-file write; no purchase commitment; no receiving action; no send-to-Amazon action; no H pause; no market proof scan; no output deletion; no live worker cycle; no scope widening; no cosmetic redesign outside the small selected-row proof drawer area; no proof-event write during render; no readiness or business-fact rewrite.
- proof_required: Focused Restock Session UI tests must pass, Streamlit Restock Session render must show the proof checklist drawer text with 0 exceptions, browser render proof should show the drawer on the current Restock Session page when available, and O MOT must keep O user-working with 0 unsafe buying actions.
...
- `python -m pytest tests\test_o_ui_operator_view.py -q -k "restock or supplier_file or card_control or supplier_proof or proof_history or missing_proof or supplier_readiness or proof_worklist or next_proof or selected_row_proof"`
- Streamlit AppTest Restock Session Supplier Review render shows selected-row proof checklist drawer text with 0 exceptions and no proof-event row writes.
...
## Expected End State
Each normal Supplier Review product card tells Luke which local proof fields matter before evidence is saved, without changing any business state.

## Proof Result
- status: proved
- compile: passed for `scripts\flows\O\O400_operator_ui.py`
- focused_tests: 30 passed, 107 deselected
...
- browser_render: opened one `Local supplier proof` drawer and showed `Selected row proof checklist`
