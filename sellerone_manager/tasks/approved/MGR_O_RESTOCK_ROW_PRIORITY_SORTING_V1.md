# O Restock Row Priority Sorting v1

## Manager Authority
- task_id: MGR_O_RESTOCK_ROW_PRIORITY_SORTING_V1
- job_ref: O-RESTOCK-ROW-PRIORITY
- flow: O
...
Add read-only row priority sorting to the Restock Session supplier review page.

This should order visible product cards so the most useful rows in the selected supplier/action bucket appear first.

## Boundary
- allowed_scope: O Restock Session card ordering, read-only use of existing row blocker/proof fields, focused UI tests, rendered page proof, O MOT proof, and active O plan notes.
...
- proof_required: Focused Restock Session UI tests must pass, Streamlit Restock Session render must show priority sorting note with 0 exceptions, browser render proof should show the current Restock Session page when available, and O MOT must keep O user-working with 0 unsafe buying actions.
...
- `python -m pytest tests\test_o_ui_operator_view.py -q -k "restock or supplier_file or card_control or supplier_proof or proof_history or missing_proof or supplier_readiness or proof_worklist or next_proof or selected_row_proof or safe_save or action_bucket or priority_sort"`
- Streamlit AppTest Restock Session Supplier Review render shows priority sorting note with 0 exceptions and no proof-event or draft-event row writes.
...
## Expected End State
The selected supplier view keeps the same rows but orders them in a more useful local work order, without changing any business state.
...
- The rendered page showed the priority sorting note in captions.
...
- Browser render proof showed the Restock Session page with the priority sorting note, local action bucket filter, safe local save guidance, and product cards.
...
## Result
The selected supplier view now keeps the same rows but places them in a better local work order. This is display-only O construction work, not a purchase-order or restock commitment.
