# O PO Draft Export Preview v1

## Manager Authority
- task_id: MGR_O_PO_DRAFT_EXPORT_PREVIEW_V1
- job_ref: O-PO-DRAFT-EXPORT-02
- flow: O
...
Add a local PO draft export preview after the PO draft review controls.

This shows what a future export packet would look like after a local review control marks a file-shape packet as shape-ready. It is still only a preview. It does not create purchase orders, write PO files, write PO hold files, commit buying, receive stock, or send anything to Amazon.

## Boundary
- allowed_scope: O PO draft export-preview contracts, local builder, UI preview view, O manager/MOT proof, focused tests, and active O plan notes.
...
- proof_required: Targeted O export-preview/session/UI/MOT tests must pass, the full local O preview builder chain must run once, and O MOT must keep O user-working with 0 unsafe buying actions.
- retest_command: python -m sellerone_manager.app --hourly-mot --mot-flow O
- rollback_path: Use git diff for code rollback. Do not delete O outputs. If local export-preview output is malformed, rebuild through O488 only after upstream local preview files are safe.
...
- `tests/test_o488_po_draft_export_preview.py`
...
## Acceptance Checks
- `python -m py_compile scripts\flows\O\O400_operator_ui.py scripts\flows\O\O488_build_po_draft_export_preview.py scripts\flows\O\O486_build_po_draft_review_controls.py scripts\flows\O\O484_build_po_preview_construction_summary.py scripts\flows\O\O482_build_po_draft_file_shape_preview.py scripts\flows\O\O480_build_po_draft_hold_review.py scripts\flows\O\O478_build_po_draft_packet_review.py scripts\flows\O\O476_build_po_line_design_preview.py scripts\flows\O\O474_build_po_draft_readiness_preview.py scripts\flows\O\O472_build_purchase_approval_guardrails.py scripts\flows\O\O470_build_purchase_approval_preview.py scripts\flows\O\_schemas.py sellerone_manager\hourly_mot.py`
- `python -m pytest tests\test_o488_po_draft_export_preview.py tests\test_o486_po_draft_review_controls.py tests\test_o484_po_preview_construction_summary.py tests\test_o482_po_draft_file_shape_preview.py tests\test_o480_po_draft_hold_review.py tests\test_o478_po_draft_packet_review.py tests\test_o476_po_line_design_preview.py tests\test_o474_po_draft_readiness_preview.py tests\test_o472_purchase_approval_guardrails.py tests\test_o470_purchase_approval_preview.py tests\test_o468_restock_pack_moq_proof_events.py tests\test_o466_restock_supplier_proof_events.py tests\test_o464_restock_supplier_batch_drafts.py tests\test_o462_restock_session_draft_decisions.py tests\test_o460_restock_session_view.py tests\test_o_ui_operator_view.py tests\manager\test_hourly_mot.py -q -k "test_o_ or operator_ui"`
...
- `python scripts\flows\O\O486_build_po_draft_review_controls.py`
- `python scripts\flows\O\O488_build_po_draft_export_preview.py`
- `python -m sellerone_manager.app --hourly-mot --mot-flow O`

## Expected End State
O can show a local PO draft export preview after a safe local review-control decision. The preview remains local-only and cannot create or commit a buy.

## Proof Result
- proved_utc: 2026-06-03T11:15:05Z
- Full compile passed for O400, O488, O486, O484, O482, O480, O478, O476, O474, O472, O470, O schemas, and O MOT.
- O-scoped proof tests passed: 134 tests, 112 deselected.
