# O Restock Approval Readiness Filter v1

## Manager Authority
- task_id: MGR_O_RESTOCK_APPROVAL_READINESS_FILTER_V1
- job_ref: O-RESTOCK-APPROVAL-READINESS
- flow: O
...
Add a read-only approval-readiness filter to the Restock Session supplier review page.

This should let Luke show only one approval-readiness lane at a time without approving buying or creating purchase orders.

## Boundary
- allowed_scope: O Restock Session supplier review display, read-only filtering from existing readiness/proof fields, focused UI tests, rendered page proof, O MOT proof, and active O plan notes.
...
- proof_required: Focused Restock Session UI tests must pass, Streamlit Restock Session render must show the approval-readiness filter with 0 exceptions, browser render proof should show the current Restock Session page when available, and O MOT must keep O user-working with 0 unsafe buying actions.
...
- Streamlit AppTest Restock Session Supplier Review render shows the approval-readiness filter with 0 exceptions and no proof-event or draft-event row writes.
...
## Expected End State
The selected supplier view can narrow product cards to one approval-readiness lane at a time, without changing rows, facts, decisions, approvals, or purchase state.
...
- The rendered page showed the approval-readiness filter.
...
- Browser render proof showed the Restock Session page with the approval-readiness filter, approval-readiness lane, supplier-info panel, and safe local save guidance.
...
## Result
The selected supplier view can now narrow rows to one approval-readiness lane at a time. This is display-only O construction work, not an approval event, purchase order, or restock commitment.
